...
function createMultiLineLabels(tLabel)
	...
	local height = (size.height+5) * lines
...
function createRichLabel(richInfo)
        ...
        if element.type == "CCLabelTTF" or element.type == "CCRenderLabel" then
            ...
            if #texts >= 2 then
                for j = 1, #texts do
                    ...
                    if j == 1 then
                        richInfo.elements[i] = newElement
                    else
                        newElement.newLine = true
                        table.insert(newElements,{index = i + j - 1, element = newElement})
                    end
                end
            end
        end
    ...
    for i = #newElements, 1, -1 do
        table.insert(richInfo.elements, newElements[i].index, newElements[i].element)
    end
    ...
    local lineIndex = 1
        for i = 1, #richInfo.elements do
            ...
            if element.type == "CCLabelTTF" or element.type == "CCRenderLabel" then
                ...
                local strokeSize = element.strokeSize == nil and 1 or element.strokeSize
                local renderType = element.renderType == nil and 1 or element.renderType
                ...
                if maxWidth == nil then
                    ...
                    elseif element.type == "CCRenderLabel" then
                        print(textTemp, fontName, fontSize, strokeSize, strokeColor, renderType)
                    ...
                    table.insert(linesInfo[lineIndex], {type = element.type, node = label})
            ...
            elseif element.type == "CCNode" or element.type == "CCMenuItem" or element.type == "CCSprite" then
                ...
                if nodeTemp:getContentSize().width > maxWidth - x then
                    lineIndex = lineIndex + 1
                    linesInfo[lineIndex] = {}
                    table.insert(linesInfo[lineIndex], {type = element.type, node = nodeTemp})
                    x = nodeTemp:getContentSize().width
                elseif nodeTemp:getContentSize().width < maxWidth - x  then
                    table.insert(linesInfo[lineIndex], {type = element.type, node = nodeTemp})
                    x = x + nodeTemp:getContentSize().width
                else
                    table.insert(linesInfo[lineIndex], {type = element.type, node = nodeTemp})
                    x = 0
                    lineIndex = lineIndex + 1
        ...
        for i = 1, lineCount do
            ...
            for j = 1, #nodes do
                ...
                if node:getContentSize().height > lineHeight then
                    lineHeight = node:getContentSize().height
                end
            end
        ...
        for i = 1, lineCount do
            ...
            for j = 1, #nodes do
                ...
                if nodes[j].type == "CCMenuItem" and menu == nil then
