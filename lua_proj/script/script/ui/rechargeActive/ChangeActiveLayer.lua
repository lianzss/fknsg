...
local function onNodeEventTip( event )
    if (event == "enter") then
        _tipLayer:registerScriptTouchHandler(onTouchesHandler, false, -5600, true)
    ...
    local function changeServiceCallBack( cbFlag, dictData, bRet )
        if(dictData.err == "ok") then
            ...
            if(dictData.ret.ret == "ok")then
                -- 增加目标兑换次数
                _serviceData.goods_list[tostring(tag)].soldNum = tonumber(_serviceData.goods_list[tostring(tag)].soldNum) + 1
                -- 兑换后扣除金币材料部分
                local desData = _serviceData.goods_list[tostring(tag)]
                -- 要兑换的公式的材料数据
                local reqItems = ActiveCache.getListReqData(desData.req)
                for i=1,#reqItems do
                    if(reqItems[i].type == "gold") then
                        -- 扣除金币
                        UserModel.addGoldNumber(-tonumber(reqItems[i].needNum))
                    end
                end
                -- 展示目标奖励
                ...
                ReceiveReward.showRewardWindow( itemData, showRewardCallFun, 1010 )
            end
        end  
    end
    -- 参数
...
function createChangeLayer( ... )
    ...
    -- 中秋大背景
    print("act_bg==>",ActivityConfig.ConfigCache.actExchange.data[1].act_bg)
    if(ActivityConfig.ConfigCache.actExchange.data[1].act_bg ~= nil and string.len(ActivityConfig.ConfigCache.actExchange.data[1].act_bg) > 0 )then
        print(type(ActivityConfig.ConfigCache.actExchange.data[1].act_bg),"0.0")
