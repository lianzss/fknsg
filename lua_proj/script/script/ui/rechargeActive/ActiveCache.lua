...
function hasTipInActive(  )
    if( isOnTime() or isMysteryNewIn() or isHaveCardNum() or isHaveVIPBenefit() or MysteryMerchant:isRefreshed() or isAllNewActivity() ) then
        return true
    else
        return false
    end
...
function isAllNewActivity( ... )
    ...
    local has= false
    local allKeys = { "robTomb" , "spend" ,"heroShop" , "signActivity","topupFund", "weal", "actExchange","groupon", "chargeRaffle"}
...
function isHaveVIPBenefit()
    ...
    if tonumber(UserModel.getVipLevel()) > 0 then
        if tostring(VIPBenefitLayer.readHave()) == "ok" then
            haveVIP = true
        end
    end
    return haveVIP
...
function getIsExtraDropAcitiveInHunt( ... )
    ...
    if(ActivityConfig.ConfigCache.actExchange.data[1])then
        ...
        if( dropId ~= nil and tonumber(dropId) > 0)then
            print("go on 4")
...
-------------------------------------------武将变身-----------------------------------
function heroTransfer(callback, hid, countryId)
    ...
    local arg = Network.argsHandler(hid, countryId)
...
function getTransferHeroesFileter()
    ...
    local heros = HeroModel.getAllHeroes()
    local transferHero = parseDB(DB_Normal_config.getDataById(1))
    for htid, hero in pairs(heros) do
...
function getTransferCost(hero)
    ...
    local cost = parseDB(DB_Normal_config.getDataById(1)).changeCardCost[index][tonumber(hero.evolve_level) + 1]
    return cost
end
