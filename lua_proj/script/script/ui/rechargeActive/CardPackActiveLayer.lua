...
function createBottom( )
	...
	-- 活动积分排行
	_rankBg= CCSprite:create(IMG_PATH .. "card_change/rank_bg.png")
	_rankBg:setPosition(18, 8)
	...
	local firstRewardText= ActiveCache.getFirstRewardText()
	local secondRewardText= ActiveCache.getSecondRewardtext()
	local thirdRewardText = ActiveCache.getThirdRewardtext()
	local forthRewardText= ActiveCache.getForthRewardtext()
...
function createRankScrollView( )
	...
	local height= 35*table.count(rank_info)
	...
	for i=1, table.count(rank_info) do 
		...
		local nameLabel = CCLabelTTF:create(rank_info[i].score , g_sFontName,21)
		nameLabel:setColor(ccc3(0x78,0x25,0x00))
		nameLabel:setAnchorPoint(ccp(0,1))
		nameLabel:setPosition( 150 ,_rankScrollView:getContentSize().height-(35)*(i)+10)
...
function createTopUI(  )
	...
	timeContent[2]= CCRenderLabel:create( TimeUtil.getTimeForDay(ActiveCache.getHeroShopStartTime()) .. GetLocalizeStringBy("key_2358"),g_sFontName,21,1,ccc3(0x00,0x00,0x0),type_stroke)
	...
	timeContent[3]= CCRenderLabel:create( "" .. TimeUtil.getTimeForDay(ActiveCache.getHeroShopEndTime()),g_sFontName,21,1,ccc3(0x00,0x00,0x0),type_stroke)
