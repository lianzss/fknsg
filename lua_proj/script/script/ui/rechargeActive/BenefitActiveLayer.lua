...
local function createScrollContent()
	...
	local explHeight = counLine(tostring(activeMes.expl))
	local desHeight  = counLine(tostring(activeMes.desc))
	...
	local EXPL = CCRenderLabel:createWithAlign(tostring(activeMes.expl), g_sFontName, 23, 1, ccc3( 0x00, 0x00, 0x00), type_stroke,CCSizeMake(405, explHeight), kCCTextAlignmentLeft, kCCVerticalTextAlignmentTop)
	...
	local DES = CCRenderLabel:createWithAlign(tostring(activeMes.desc), g_sFontName, 23, 1, ccc3( 0x00, 0x00, 0x00), type_stroke,CCSizeMake(405, desHeight), kCCTextAlignmentLeft, kCCVerticalTextAlignmentTop)
...
function getFormatYMDHM(typeNum)
	...
	return tostring(year) .. "年" .. tostring(month) .. "月" .. tostring(day) .. "日" .. " " .. tostring(hour) .. ":" .. tostring(min)
end