...
function createCell(tCellValues)
	...
	if(isHaveGet)then
		...
		local itemFont = CCRenderLabel:create(fontStr,  g_sFontPangWa, 35, 1, ccc3(0x00, 0x00, 0x00), type_stroke)
		...
		item:addChild(itemFont,1,123)
		-- 判断是否够资格领取
		local dayNum = ActiveCache.getNewYearSignDayNum()
		if(dayNum < tonumber(tCellValues.accumulateDay))then
			-- 登陆天数不够 不够资格领取 按钮不可点且置灰
			item:setEnabled(false)
			itemFont:setColor(ccc3(0xf1,0xf1,0xf1))
		end
	end
