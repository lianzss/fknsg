...
local function createRewardList()
    ...
    _listWidth = 635
    listBg:setContentSize(CCSizeMake(_listWidth,_listHight/g_fScaleX))
    listBg:setAnchorPoint(ccp(0.5,1))
    local posY = _buyLabelNode:getPositionY()-_buyLabelNode:getContentSize().height*g_fScaleX-20*g_fScaleX
    ...
    local cellSize = CCSizeMake(612,232)
...
local function buyItemCallFun( tag, itemBtn )
    ...
    local function nextCallFun( ... )
        ...
        itemSp:setPosition(ccp(458, _sellItemBg:getContentSize().height*0.5))
...
local function createSellItem()
    ...
    _sellItemBg:setContentSize(CCSizeMake(614,204))
    ...
    goodsBg:setContentSize(CCSizeMake(413,147))
    goodsBg:setAnchorPoint(ccp(0,0))
    goodsBg:setPosition(ccp(23,20))
    _sellItemBg:addChild(goodsBg)
    -- 物品icon
    local iconBg = CCSprite:create("images/everyday/headBg1.png")
    iconBg:setAnchorPoint(ccp(0,0.5))
    iconBg:setPosition(ccp(13,goodsBg:getContentSize().height*0.56))
    goodsBg:addChild(iconBg)
    -- 图标底
    ...
    font1:setPosition(ccp(44,_sellItemBg:getContentSize().height-15))
    ...
    if(haveNum == 0)then
        ...
        menuItem:setPosition(ccp(458, _sellItemBg:getContentSize().height*0.5))
...
local function createMenuItem(  )
	...
	local btn_size_n	= CCSizeMake(150, 45)
	local btn_size_h	= CCSizeMake(158, 55)
...
local function timeDownFun( ... )
    ...
    if(_listTableView ~= nil)then
        local offset =  _listTableView:getContentSize().height+ _listTableView:getContentOffset().y-(_listHight/g_fScaleX-22)
...
local function initTuanActiveLayer( ... )
    ...
    _topBg:setPosition(ccp(_bgLayer:getContentSize().width*0.5,_bgLayer:getContentSize().height-topMenuHeight))
    ...
    timeFont:setPosition(ccp(178,95))
    ...
    _timeFont:setPosition(ccp(font1:getPositionX()+2,font1:getPositionY()))
