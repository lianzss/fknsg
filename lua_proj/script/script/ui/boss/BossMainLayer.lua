...
-- --青龙
--     local boss = CCSprite:create("images/boss/boss.png")
--     boss:setAnchorPoint(ccp(0.5, 0.5))
--     bossLayer:addChild(boss)
--     boss:setScale(g_fElementScaleRatio)
--     boss:setPosition(ccp(winSize.width/2, winSize.height/2 + 100))

 -- 动画
    local loadEffectSprite = CCLayerSprite:layerSpriteWithName(CCString:create("images/boss/effect/wordboss"), -1,CCString:create(""));
...
-- 返回
    ...
    backItem:setPosition(ccp(1, 1))
    backItem:setAnchorPoint(ccp(0,0.5))
    backItem:setScale(g_fElementScaleRatio)
    local backMenu = CCMenu:createWithItem(backItem)
    bossLayer:addChild(backMenu)
    backMenu:setPosition(ccp(winSize.width - backItem:getContentSize().width*g_fElementScaleRatio*1.5, winSize.height - BulletinLayer.getLayerHeight()*g_fScaleX - bossLayer:getChildByTag(19876):getContentSize().height*bossLayer:getChildByTag(19876):getScale() - backItem:getContentSize().height*g_fElementScaleRatio))
    ...
    rankItem:setPosition(ccp(1, 1))
    rankItem:setAnchorPoint(ccp(0,0.5))
    rankItem:setScale(g_fElementScaleRatio)
    local rankMenu = CCMenu:createWithItem(rankItem)
    rankMenu:setPosition(ccp(backItem:getContentSize().width*g_fElementScaleRatio*0.5, winSize.height - BulletinLayer.getLayerHeight()*g_fScaleX - bossLayer:getChildByTag(19876):getContentSize().height*bossLayer:getChildByTag(19876):getScale() - backItem:getContentSize().height*g_fElementScaleRatio))
    
    bossLayer:addChild(rankMenu, 100, 909098)
    rankMenu:setTouchPriority(- 500)

    local icon = CCSprite:create("images/boss/boss_name.png")
    bossLayer:addChild(icon)
    icon:setAnchorPoint(ccp(0.5,0.5))
    icon:setScale(g_fElementScaleRatio)
    icon:setPosition(ccp(winSize.width/2, winSize.height - icon:getContentSize().height*g_fElementScaleRatio*2))
...
--名字背景
    ...
    nameBg:setPosition(ccp(winSize.width/2, 450*g_fElementScaleRatio))
    bossLayer:addChild(nameBg, 1)

-- 名字等级
    local nameLabel = CCRenderLabel:create(GetLocalizeStringBy("key_1283"), g_sFontPangWa, 25, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    ...
    lvSprite:setPosition(ccp(nameBg:getContentSize().width/2 + 10, nameBg:getContentSize().height/2))
    ...
    lvLabel:setPosition(ccp(nameBg:getContentSize().width/2 + 60, nameBg:getContentSize().height/2))
...
--描述label1
    ...
    labelTabel1[2] = CCRenderLabel:create(GetLocalizeStringBy("key_1283"), g_sFontPangWa, 25, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    ...
    normalDesNode1:setPosition(winSize.width/2, 410*g_fElementScaleRatio)
    normalDesNode1:setScale(g_fElementScaleRatio)
    bossLayer:addChild(normalDesNode1)

--描述label2
    local labelTabel2 = {}
    labelTabel2[1] = CCRenderLabel:create(GetLocalizeStringBy("lic_1131"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[1]:setColor(ccc3(0x00, 0xff, 0x18))
    
    labelTabel2[2] = CCRenderLabel:create(GetLocalizeStringBy("lic_1132"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[2]:setColor(ccc3(0xf0, 0x45, 0xff))

    labelTabel2[3] = CCRenderLabel:create(GetLocalizeStringBy("lic_1133"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[3]:setColor(ccc3(0x00, 0xff, 0x18))

    labelTabel2[4] = CCRenderLabel:create(GetLocalizeStringBy("lic_1134"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[4]:setColor(ccc3(0xf9, 0x59, 0xff))

    labelTabel2[5] = CCRenderLabel:create(GetLocalizeStringBy("lic_1135"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[5]:setColor(ccc3(0x00, 0xff, 0x18))

    labelTabel2[6] = CCRenderLabel:create(GetLocalizeStringBy("lic_1136"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[6]:setColor(ccc3(0xf9, 0x59, 0xff))

    labelTabel2[7] = CCRenderLabel:create(GetLocalizeStringBy("lic_1137"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel2[7]:setColor(ccc3(0x00, 0xff, 0x18))

    local normalDesNode2 = BaseUI.createHorizontalNode(labelTabel2)
    normalDesNode2:setAnchorPoint(ccp(0.5, 0.5))
    normalDesNode2:setPosition(winSize.width/2, 370*g_fElementScaleRatio)
    normalDesNode2:setScale(g_fElementScaleRatio)
    bossLayer:addChild(normalDesNode2)

--描述label3
    local labelTabel3 = {}
    labelTabel3[1] = CCRenderLabel:create(GetLocalizeStringBy("lic_1138"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel3[1]:setColor(ccc3(0x00, 0xff, 0x18))
    
    labelTabel3[2] = CCRenderLabel:create(GetLocalizeStringBy("lic_1139"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel3[2]:setColor(ccc3(0xf0, 0x45, 0xff))

    labelTabel3[3] = CCRenderLabel:create(GetLocalizeStringBy("lic_1140"), g_sFontPangWa, 21, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    labelTabel3[3]:setColor(ccc3(0x00, 0xff, 0x18))

    local normalDesNode3 = BaseUI.createHorizontalNode(labelTabel3)
    normalDesNode3:setAnchorPoint(ccp(0.5, 0.5))
    normalDesNode3:setPosition(winSize.width/2, 330*g_fElementScaleRatio)
    normalDesNode3:setScale(g_fElementScaleRatio)
    bossLayer:addChild(normalDesNode3)

--击杀排名，击杀着
    -- local fullRect = CCRectMake(0, 0, 169, 37)
    -- local insetRect = CCRectMake(40, 15, 1, 1)
    -- local lastBg = CCScale9Sprite:create("images/boss/last_rank_bg.png", fullRect, insetRect)
    -- lastBg:setPreferredSize(CCSizeMake(winSize.width, 50))
    -- lastBg:setAnchorPoint(ccp(0.5, 0.5))
    -- lastBg:setScale(g_fElementScaleRatio)
    -- lastBg:setPosition(ccp(winSize.width/2, 270*g_fElementScaleRatio))
    -- bossLayer:addChild(lastBg)
    ...
    -- local threeString = threePlayer[1].name..","..threePlayer[2].name..","..threePlayer[3].name
    ...
    -- firstThreeLabel:setColor(ccc3(0x00, 0xff, 0xa8))
    ...
    -- lastBg:addChild(firstThreeLabel)
    ...
    -- killerLabel:setColor(ccc3(0x00, 0xff, 0xa8))
    ...
    -- 时间
    local beginTime = CCSprite:create("images/boss/boss_time.png")
    bossLayer:addChild(beginTime)
    beginTime:setAnchorPoint(ccp(0.5, 0.5))
    beginTime:setPosition(ccp(winSize.width/2, 200*g_fElementScaleRatio))
    beginTime:setScale(g_fElementScaleRatio)
...
--真正的时间戳
    ...
    beginTime:addChild(timeDataLabel, 1)
    timeDataLabel:setAnchorPoint(ccp(0.5, 0.5))
    timeDataLabel:setPosition(ccp(beginTime:getContentSize().width/2, beginTime:getContentSize().height/2))
...
--得到时间戳
    ...
    -- CCRenderLabel:create(leftTime, g_sFontPangWa, 23, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    ...
    -- local rankItem = CCMenuItemImage:create("images/common/btn/btn_hurt_n.png", "images/common/btn/btn_hurt_h.png")
    -- rankItem:registerScriptTapHandler(fightRank)
    -- rankItem:setAnchorPoint(ccp(0,1))
    -- rankItem:setScale(g_fElementScaleRatio)

    -- local rankMenu = CCMenu:createWithItem(rankItem)
    -- --local rankMenu = superHeroMenu()
    -- bossLayer:addChild(rankMenu, 100, 909098)
    -- rankMenu:setPosition(ccp(rankItem:getContentSize().width*g_fElementScaleRatio*1.5, winSize.height - BulletinLayer.getLayerHeight()*g_fScaleX - bossLayer:getChildByTag(19876):getContentSize().height*bossLayer:getChildByTag(19876):getScale() - rankItem:getContentSize().height*g_fElementScaleRatio*1.5))
    -- --rankMenu:setPosition(winSize.width/2, 60*g_fElementScaleRatio)
    -- --rankMenu:alignItemsHorizontallyWithPadding(80*g_fElementScaleRatio)
    -- rankMenu:setTouchPriority(- 500)
    -- rankMenu:setScale(g_fElementScaleRatio)
end


function superHeroMenu()
    --武将和伤害按钮
    -- require "script/libs/LuaCC"
    -- local specialItem = LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png", CCSizeMake(198,73), GetLocalizeStringBy("key_2076"), ccc3(255,222,0))
    -- specialItem:registerScriptTapHandler(specialPlayer)
    -- -- specialItem:registerScriptTapHandler(settlePanel)

    -- specialItem:setPosition(winSize.width*0.3, 0)
    -- --specialItem:setPosition(winSize.width*0.5, winSize.height*0.25)
    -- specialItem:setAnchorPoint(ccp(0.5, 0.5))
    -- specialItem:setScale(g_fElementScaleRatio)

    local rankItem = CCMenuItemImage:create("images/common/btn/btn_hurt_n.png", "images/common/btn/btn_hurt_h.png")
    rankItem:registerScriptTapHandler(fightRank)
    rankItem:setPosition(winSize.width*0.15, winSize.height*0.78)
    rankItem:setAnchorPoint(ccp(0.5, 0.5))
    rankItem:setScale(g_fElementScaleRatio)

    local menuArray = CCArray:create()
    -- menuArray:addObject(specialItem)
    menuArray:addObject(rankItem)

    local rankMenu = CCMenu:createWithArray(menuArray)
    -- rankMenu:setTouchPriority(- 300)
...
function specialPlayer()
    ...
    local titleLabel = CCRenderLabel:create(GetLocalizeStringBy("key_2076"), g_sFontPangWa, 35, 1, ccc3(0,0,0))
    ...
    --提示
    local tipLabel1 = CCRenderLabel:create(GetLocalizeStringBy("key_2536"), g_sFontPangWa, 24, 1, ccc3(0xff, 0xff, 0xff))
    tipLabel1:setPosition(ccp(background:getContentSize().width/2, background:getContentSize().height - 81))
    tipLabel1:setColor(ccc3(0x78, 0x25, 0x00))
    tipLabel1:setAnchorPoint(ccp(0.5, 0))
    background:addChild(tipLabel1)


    local tipLabel3 = CCRenderLabel:create(GetLocalizeStringBy("key_2643"), g_sFontPangWa, 24, 1, ccc3(0xff, 0xff, 0xff))
    tipLabel3:setPosition(ccp(background:getContentSize().width/2, background:getContentSize().height - 110))
    tipLabel3:setColor(ccc3(0x78, 0x25, 0x00))
    tipLabel3:setAnchorPoint(ccp(0.5, 0))
    background:addChild(tipLabel3)

    local tipLabel2 = CCRenderLabel:create(GetLocalizeStringBy("key_3045"), g_sFontPangWa, 24, 1, ccc3(0xff, 0xff, 0xff))
    tipLabel2:setPosition(ccp(background:getContentSize().width/2, background:getContentSize().height - 140))
...
--关闭按钮
    ...
    closeMenu:setTouchPriority(-5040)


    BossNet.getSpecialPlayer(function ( ... )
         --里面的内容
        --bg
        ...
        listBg:setPreferredSize(CCSizeMake(585, 600))
        listBg:setPosition(ccp(25, 45))
        background:addChild(listBg)
        
        
--特殊武将的个数
        local heroinfo = DB_Worldboss.getDataById(1).ratioHerosNums

        local inspireArr = string.split(heroinfo, ",")
        local heroNumber1 = string.split(inspireArr[1], "|")
        local heroNum1 = heroNumber1[1]
        
        local heroNumber2 = string.split(inspireArr[2], "|")
        local heroNum2 = heroNumber2[1]

        local heroNumber3 = string.split(inspireArr[3], "|")
        local heroNum3 = heroNumber3[1]

        -- print("heroNum1 :::::::::  "..math.ceil(heroNum1/3) .." d f "..math.ceil(heroNum2/3) .."dsfgas  "..math.ceil(heroNum3/3) )

    --scrollview
        local scrollView = CCScrollView:create()
        scrollView:setViewSize(CCSizeMake(570, 580))
        scrollView:setTouchPriority(-5040)
        scrollView:setContentSize(CCSizeMake(570, 125 * 3 + 150*(math.ceil(heroNum1/3) + math.ceil(heroNum2/3) + math.ceil(heroNum3/3))))
        ...
        local firstHeroList = createSuperHero(3)
        contentBg:addChild(firstHeroList)
        firstHeroList:setPosition(ccp(0, scrollView:getContentSize().height - firstHeroList:getContentSize().height))

        local secondHeroList = createSuperHero(2)
        contentBg:addChild(secondHeroList)
        secondHeroList:setPosition(ccp(0, scrollView:getContentSize().height - firstHeroList:getContentSize().height - secondHeroList:getContentSize().height ))

        local thirdHeroList = createSuperHero(1)
        contentBg:addChild(thirdHeroList)
        thirdHeroList:setPosition(ccp(0, scrollView:getContentSize().height - firstHeroList:getContentSize().height - secondHeroList:getContentSize().height - thirdHeroList:getContentSize().height))

    end)
end


function createSuperHero(tag)
    local nodeBg = CCNode:create()
    nodeBg:setAnchorPoint(ccp(0, 0))
    local heroArray = BossData.superHeroInfo.good
    local times = "100%"
    local playerTimes = GetLocalizeStringBy("key_2688")
    if(tag == 2) then
        times = "200%"
        playerTimes = GetLocalizeStringBy("key_2562")
        heroArray = BossData.superHeroInfo.better
    elseif(tag == 3) then
        times = "400%"
        playerTimes = GetLocalizeStringBy("key_1366")
        heroArray = BossData.superHeroInfo.best
    end
    -- nodeBg:setContentSize(CCSizeMake(585, (table.count(heroArray)-1)/3 + 1)*170)
    print("heroArray is >>>>> ")
    print_t(heroArray)
    -- nodeBg:setContentSize(CCSizeMake(585, ((table.count(heroArray) - 1)/3 + 1)*170 + 60))
    nodeBg:setContentSize(CCSizeMake(585, 120 + 150))


    print("the node bg contentsize is >>>  "..nodeBg:getContentSize().height)
    for i,v in ipairs(heroArray) do
        print("the heroArray is ")
        print(i,v)
        require "script/battle/BattleCardUtil"
        local dressId = HeroModel.getNecessaryHero().equip.dress["1"]
        print("the dress id is ",dressId)

        if(dressId and tonumber(dressId) ~= 0) then
            dressId = dressId.item_template_id 
        else
            dressId = nil
        end
        print("the dress id is >>>>",dressId)
        ...
        local hero = BattleCardUtil.getFormationPlayerCard(0, nil, v, dressId)
        nodeBg:addChild(hero)
        hero:setAnchorPoint(ccp(0, 1))
        hero:setPosition(ccp(35 + (i-1)*195, nodeBg:getContentSize().height - 100))
        if(i > 3) then
            hero:setPosition(ccp(35 + ((i-1)%3)*195, nodeBg:getContentSize().height - 100 - 190))
        end

        require "script/ui/formation/FormationUtil"
        local heroSelect = FormationUtil.isHadSameTemplateOnFormationByHtid(v)
        print("the heroSelect is ", heroSelect)
        if(heroSelect) then
            --判断当前阵型上 是否有武将
            print("hero is selected ")
            local heroSelectBg = CCSprite:create("images/boss/hero_ison.png")
            heroSelectBg:setAnchorPoint(ccp(0.5, 0.5))
            heroSelectBg:setPosition(ccp(hero:getContentSize().width/2, hero:getContentSize().height/2))
            hero:addChild(heroSelectBg, 10)
        end
      
        require "db/DB_Heroes"
        local heroInfo = DB_Heroes.getDataById(v)
        local nameLabel = CCRenderLabel:create(heroInfo.name, g_sFontPangWa, 24, 1, ccc3(0,0,0))
        nameLabel:setPosition(ccp(hero:getContentSize().width/2, 0))
        
        local heroDB = DB_Heroes.getDataById(v)
        local heroColor = HeroPublicLua.getCCColorByStarLevel(heroDB.star_lv)
        nameLabel:setColor(heroColor)

        nameLabel:setAnchorPoint(ccp(0.5, 1))
        hero:addChild(nameLabel)

        if(table.count(heroArray) == 1) then
            hero:setAnchorPoint(ccp(0.5, 1))
            hero:setPosition(ccp(nodeBg:getContentSize().width/2, nodeBg:getContentSize().height - 100))
        elseif(table.count(heroArray) == 2) then
            hero:setAnchorPoint(ccp(0.5, 1))
            hero:setPosition(ccp(nodeBg:getContentSize().width/3 * i, nodeBg:getContentSize().height - 100))
        end

        print("the hero position is ... "..hero:getPositionX().." d safn "..hero:getPositionY())
    end

    local titleBg = CCSprite:create("images/common/star_bg.png")
    nodeBg:addChild(titleBg)
    titleBg:setAnchorPoint(ccp(0.5, 0.5))
    titleBg:setPosition(nodeBg:getContentSize().width/2, nodeBg:getContentSize().height - 25)

    local titles = playerTimes..GetLocalizeStringBy("key_2867")
    print("the times is >>>>> ".. titles)
    local tipLabel1 = CCRenderLabel:create(titles, g_sFontPangWa, 24, 1, ccc3(0,0,0))
    tipLabel1:setPosition(ccp(titleBg:getContentSize().width/2, titleBg:getContentSize().height/2))
    tipLabel1:setColor(ccc3(0xff, 0xe4, 0x00))
    tipLabel1:setAnchorPoint(ccp(0.5, 0.5))
    titleBg:addChild(tipLabel1)

    local title2 = GetLocalizeStringBy("key_2245")..times
    local tipLabel2 = CCRenderLabel:create(title2, g_sFontName, 18, 1, ccc3(0x00, 0x00, 0x00))
    tipLabel2:setPosition(ccp(nodeBg:getContentSize().width/2, nodeBg:getContentSize().height - 60))
    tipLabel2:setColor(ccc3(0xff, 0xe4, 0x00))
    tipLabel2:setAnchorPoint(ccp(0.5, 0.5))
    nodeBg:addChild(tipLabel2)
    ...
    line:setPreferredSize(CCSizeMake(580, 3))
    line:setPosition(ccp(10, - 10))
    nodeBg:addChild(line)
...
function fightRank( ... )
    ...
    BossNet.getRankList(function ()
        ...
        scrollView:setTouchPriority(-5030)
...
--内容bg
    ...
    --内容
        ...
        --伤害
            ...
            formationMenu:setTouchPriority(-600)
...
--开始前
    print("before boss fight")
        ...
        if(tonumber(bTime - nowTime) == 0) then
            ...
            enterMenu:setTouchPriority(-5001)
...
--播放音乐
    local music = DB_Worldboss.getDataById(1).backGroundMusic
...
--青龙
    ...
    -- boss:setAnchorPoint(ccp(0.5, 0.5))
    -- bossLayer:addChild(boss)
    -- boss:setScale(g_fBgScaleRatio)
    -- boss:setPosition(ccp(winSize.width/2, winSize.height/2))
--青龙特效
 -- 动画
    local loadEffectSprite = CCLayerSprite:layerSpriteWithName(CCString:create("images/boss/effect/wordboss"), -1,CCString:create(""));
    loadEffectSprite:retain()
...
--顶部
    -- local fullRect = CCRectMake(0, 0, 640, 98)
    -- local insetRect = CCRectMake(30, 30, 15, 10)
    -- local topBg = CCScale9Sprite:create("images/formation/changeequip/topbg.png", fullRect, insetRect)
    -- topBg:setPreferredSize(CCSizeMake(640*g_fScaleX, 1))
    -- -- local topBg = CCSprite:create("images/formation/changeequip/topbg.png")
    -- topBg:setPosition(ccp(0, winSize.height - 80*g_fElementScaleRatio))
    -- topBg:setAnchorPoint(ccp(0, 1))
    -- bossLayer:addChild(topBg)
    ...
    backItem:setScale(g_fElementScaleRatio)
    local backMenu = CCMenu:createWithItem(backItem)
    bossLayer:addChild(backMenu, 99)
    backMenu:setPosition(ccp(winSize.width - backItem:getContentSize().width*1.1*g_fElementScaleRatio, winSize.height - backItem:getContentSize().height*3*g_fElementScaleRatio))

    local rankItem = CCMenuItemImage:create("images/common/btn/btn_hurt_n.png", "images/common/btn/btn_hurt_h.png")
    rankItem:registerScriptTapHandler(fightRank)
    rankItem:setPosition(ccp(0, 0))
    rankItem:setAnchorPoint(ccp(0,0.5))
    rankItem:setScale(g_fElementScaleRatio)
    local rankMenu = CCMenu:createWithItem(rankItem)
    bossLayer:addChild(rankMenu)
    rankMenu:setPosition(ccp(backItem:getContentSize().width*0.1*g_fElementScaleRatio, winSize.height - backItem:getContentSize().height*3*g_fElementScaleRatio))


    -- rankMenu:setAnchorPoint(ccp(0.5, 0.5))
    -- bossLayer:addChild(rankMenu)
    -- rankMenu:setPosition(0,0)
    --rankMenu:setPosition(topBg:getContentSize().width/2 - 30*g_fElementScaleRatio, topBg:getContentSize().height/2)
    --rankMenu:alignItemsHorizontallyWithPadding(40*g_fElementScaleRatio)
-- 血量
...
--名字
    local nameLabel = CCRenderLabel:create(DB_Worldboss.getDataById(1).name, g_sFontPangWa, 23, 1, ccc3(0,0,0))
...
function silverInspire()
    ...
    BossNet.silverInspire(function ( ... )
        ...
        m_silverLabel:setString(UserModel.getSilverNumber())
...
function settlePanel(returnBoss)
    --遮挡层
    local maskLayer = BaseUI.createMaskLayer(-5000)
...
--[[
    ...
    local coinData = DB_Worldboss.getDataById(1).attackSilver or 0
...
--设置数值,银币，将魂
    -- UserModel.addSilverNumber(soulData)
    -- m_silverLabel:setString(UserModel.getSilverNumber())
...
function fightBoss( ... ) 
    ...
    BossNet.attackBoss(function ( ... )
        ...
        -- local fightCD = DB_Worldbossinspire.getDataById(1).cd
        -- leftFightCD = tonumber(fightCD - leftCD)
    ...
    --当前排名
        local rankData = BossData.attackData.rank
...
function setAutoFight(item, tag)
    ...
    print("the vipArr is ")
    print_t(vipArr)
    local vipBoss = vipArr[1].worldbossAutoAttack
    print("the vipBoss is ",vipBoss)
    if(tonumber(vipBoss) == 0) then
        AnimationTip.showTip(GetLocalizeStringBy("key_1516"))
        return
    end
...
--分两种情况，是不是击杀
    ...
    -- 彩带--标题
        ...
        -- labelTabel1[2] = CCRenderLabel:create(DB_Worldboss.getDataById(1).name, g_sFontPangWa, 25, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
        -- labelTabel1[2]:setColor(ccc3(0x00, 0xe4, 0xff))

        -- labelTabel1[3] = CCRenderLabel:create(GetLocalizeStringBy("key_1925"), g_sFontPangWa, 25, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
        -- labelTabel1[3]:setColor(ccc3(0xff, 0xe4, 0x00))
    ...
    --确定按钮
        ...
        closeMenu:addChild(closeButton, 9999)
        closeMenu:setTouchPriority(-504000)
        ...
        -- listBg:setPreferredSize(CCSizeMake(440, 560))
...
--判断是否击杀
        ...
        -- if(tonumber(killer) == 1) then
        --     KillerTitle = GetLocalizeStringBy("key_1713")
        -- end
...
--[[
    ...
    -- 奖励
        ...
        local tipLabel1 = CCRenderLabel:create(GetLocalizeStringBy("key_1809"), g_sFontPangWa, 24, 1, ccc3(0,0,0))
        tipLabel1:setPosition(ccp(titleBg1:getContentSize().width/2, titleBg1:getContentSize().height/2))
        tipLabel1:setColor(ccc3(0xff, 0xe4, 0x00))
        tipLabel1:setAnchorPoint(ccp(0.5, 0.5))
        titleBg1:addChild(tipLabel1)
    ...
    --获得的银币和将魂
        ...
        local silverLabel = CCLabelTTF:create(GetLocalizeStringBy("key_2470"), g_sFontName, 24)
        -- CCRenderLabel:create(GetLocalizeStringBy("key_2470"), g_sFontName, 24, 1, ccc3(0,0,0))
        silverLabel:setPosition(ccp(titleBg1:getContentSize().width/2, titleBg1:getContentSize().height/2))
        silverLabel:setColor(ccc3(0x78, 0x25, 0x00))
        silverLabel:setAnchorPoint(ccp(0.5, 0.5))
        silverBg:addChild(silverLabel)
        ...
        local soulData = BossData.prizeData.reward_rank.soul or 0
        local soulDataLabel = CCLabelTTF:create(soulData, g_sFontName, 24)
        soulDataLabel:setColor(ccc3(0x00, 0x00, 0x00))
        soulBg:addChild(soulDataLabel)
        soulDataLabel:setPosition(ccp(230, soulBg:getContentSize().height/2))
    ...
    -- 战利品
        ...
        local tipLabel2 = CCRenderLabel:create(GetLocalizeStringBy("key_2882"), g_sFontPangWa, 24, 1, ccc3(0,0,0))
        tipLabel2:setPosition(ccp(titleBg2:getContentSize().width/2, titleBg2:getContentSize().height/2))
        tipLabel2:setColor(ccc3(0xff, 0xe4, 0x00))
        tipLabel2:setAnchorPoint(ccp(0.5, 0.5))
        titleBg2:addChild(tipLabel2)

        local prizeItemArray = BossData.prizeData.reward_rank.arrItemTpl
        if(table.isEmpty(prizeItemArray)) then
            return
        end
        print_t(prizeItemArray)
        for k,v in pairs(prizeItemArray) do
            require "script/ui/item/ItemSprite"
            require "script/ui/item/ItemUtil"
            require "script/ui/hero/HeroPublicLua"
            print("the ")
            local itemIcon = ItemSprite.getItemSpriteById(k, nil, nil, nil, -600,999999)
            listBg:addChild(itemIcon)
            itemIcon:setAnchorPoint(ccp(0, 0))
            itemIcon:setPosition(ccp(60, 150))

            local itemName = ItemUtil.getItemById(k)
            local itemNameLabel = CCRenderLabel:create(itemName.name, g_sFontName, 18, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
            itemNameLabel:setColor(ccc3(0xff, 0xe4, 0x00))
            itemNameLabel:setAnchorPoint(ccp(0.5,0.5))
            itemNameLabel:setPosition(ccp(itemIcon:getContentSize().width*0.5, - 10))
...
function createTopUI( ... )
    ...
    m_silverLabel = CCLabelTTF:create(tostring(userInfo.silver_num),g_sFontName,18)
