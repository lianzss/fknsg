...
local function showNext()
    if(getNextDialog() ~= true) then
        ...
        -- if(m_currentDialog.option.data ~= nil and string.sub(m_currentDialog.option.data, 1, 3)=="11#")then
        --     local strongholdId = string.sub(m_currentDialog.option.data, 4, string.len(m_currentDialog.option.data))
        --     --打开新的据点
            
        --     require "db/DB_Stronghold"
        --     local sh = DB_Stronghold.getDataById(strongholdId)
            
        --     if(sh~=nil)then
        --         require "script/ui/copy/CopyUtil"
        --         local progress = CopyUtil.getStrongholdProgress(sh.copy_id,sh.id)
        --         if(progress~=nil)then
        --             require "script/ui/copy/FortInfoLayout"
        --             local fiLayer = FortInfoLayout.createLayer(sh.copy_id,sh.id,progress)
        --             MainScene.changeLayer(fiLayer, "fortInfoLayer")
        --         end
        --     end
        -- end
    ...
    if(m_currentDialog.leftHeaderId~=nil and tonumber(m_currentDialog.leftHeaderId)~=0) then
        ...
        --local headerId = m_currentDialog.leftHeaderId==999 and UserModel.getUserInfo().htid or m_currentDialog.leftHeaderId
        
        --require "script/model/utils/HeroUtil"
        --local imageFile = HeroUtil.getHeroHalfLenImageStringByHTID(tonumber(headerId))
    ...
    if(m_currentDialog.rightHeaderId~=nil and tonumber(m_currentDialog.rightHeaderId)~=0) then
        ...
        --require "script/model/user/UserModel"
        --local headerId = m_currentDialog.rightHeaderId==999 and UserModel.getUserInfo().htid or m_currentDialog.rightHeaderId
        ...
        if(tonumber(m_currentDialog.rightHeaderId)==999) then
            ...
            --print("m_currentDialog.rightHeaderId:      ",headId)
            --print("type(headId): ", type(headId))
...
function createTalkLayer(talkID)
    ...
    --local bgSize = CCSizeMake(640,220)
    --local scale = CCDirector:sharedDirector():getWinSize().width/bgSize.width
    
    --m_labelBg = CCScale9Sprite:create(CCRectMake(235, 50, 20, 80),IMG_PATH .. "dialogbg.png")
    --m_labelBg:setContentSize(bgSize)
    ...
    m_dialogLabel = CCLabelTTF:create("一二三四五六七八九十一二三四五六七八九十一二三四五六七八九十一二三四五六七八九十",g_sFontName,28,CCSizeMake(bgSize.width*0.9, bgSize.height*0.75),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
    ...
    m_baseLayer:registerScriptTouchHandler(layerTouch,false,-999,true)
