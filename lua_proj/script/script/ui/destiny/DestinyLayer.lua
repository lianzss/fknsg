...
function createMiddleUI(  )
	...
	backBtn:setPosition(ccp(_layerSize.width-10, height-22*MainScene.elementScale ))
	...
	learnSkillBtn:setPosition(_layerSize.width-115*g_fScaleX, height-14*MainScene.elementScale )
...
function createHeroSp( )
	...
	if( DestinyData.getCurDestiny() > DestinyData.getFirstBreakId()) then
...
function activatCallback( cbFlag, dictData, bRet )
	...
	local function starActionCallBack( ... )
		...
		if( DestinyData.isBreak() == true) then		
			...
			-- print_t(heroData)
			local fightForce= HeroFightForce.getAllForceValuesByHid(heroData.hid )
			...
			-- print_t(heroData_2)
			local fightForce_2= HeroFightForce.getAllForceValuesByHid(heroData_2.hid )
...
function playStarEffect( beginNode, endNode, callBackFunc )
	...
	local createStar = function ( i )
	    ...
	    local bezierTo 		  = CCBezierTo:create(0.8 + 0.4 * i ,bezier)
...
function fnCreateTransferEffect(pForceValues01, pForceValues02)
	...
	local values_02 = {
		pForceValues02.magicDefend,
		 pForceValues02.physicalDefend,
		 pForceValues02.generalAttack,
		 pForceValues02.life,
