...
local _touch_priority       = -500  -- 当前层触摸优先级
local _cols                         -- 总列数
local _rows                         -- 总行数
...
local _speed                = 1     -- 人物的移动速度，每1秒移动100像素
...
function show()
    ...
    -- 拉取阵型
    RequestCenter.dragonGetUserBf(handleFormation, nil)
    -- 拉取地图
    RequestCenter.dragonGetMap(handleGetMap, nil)
end
...
function createMenu()
    ...
    _menu:setTouchPriority(_touch_priority - 30)
...
function refreshBtn()
    ...
    if _map_info.free_reset_num > 0 then
        ...
        tip_sprite = LuaCCSprite.createTipSpriteWithNum(_map_info.free_reset_num)
    else
...
function autoCallback()
    ...
    local floor_name = string.format(GetLocalizeStringBy("key_8117"), GetLocalizeStringBy(chinese_number[ _selected_floor_index]))
    ...
    --todo
    --[[
    require "script/ui/forge/AutoFindLayer"
    AutoFindLayer.show()
    --]]
end
...
function autoFind(isConfirm, _argsCB)
    ...
    local args = Network.argsHandler(_selected_floor_index, _selected_box_index - 1)
...
function handleAuto(cbFlag, dictData, bRet)
    ...
    RequestCenter.dragonGetMap(handleGetMap, nil)
    UserModel.addGoldNumber(-_auto_gold_count)
    require "script/ui/forge/AutoFindResultLayer"
    local data = dictData.ret
    data.act = _auto_act_count
    data.selected_box_index = _selected_box_index
    data.selected_floor_start_index = _selected_floor_index
    data.selected_floor_end_index = _selected_floor_index
...
function createTopNode()
    ...
    local title_sprite = CCSprite:create("images/forge/title.png")
    _top_node:addChild(title_sprite)
    title_sprite:setAnchorPoint(ccp(0.5, 0.5))
    title_sprite:setPosition(105, 52)
...
function refreshMap()
    ...
    _rows = FindTreasureUtil.getRows()
    _cols = FindTreasureUtil.getCols()
...
function refreshFloor()
    ...
    local chinese_number = {"key_8107", "key_8108", "key_8109", "key_8110", "key_8111", "key_8112", "key_8113", "key_8114", "key_8115", "key_8116"}
    _floor_label:setString(string.format(GetLocalizeStringBy("key_8117"), GetLocalizeStringBy(chinese_number[ _map_info.floor])))
end
...
function buyAct(count, total_gold_count)
    ...
    local args = Network.argsHandler("0", tostring(count))
     RequestCenter.dragonBuyAct(handleBuyAct, args)
     _buy_act_count = count
     _buy_act_total_gold_count = total_gold_count
end

-- 购买行动力网络回调
function handleBuyAct(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    UserModel.addGoldNumber(-_buy_act_total_gold_count)
    _map_info.act = _map_info.act + _buy_act_count
    _map_info.buyactnum = _map_info.buyactnum + _buy_act_count
    refreshAct()
...
-- 刷新血槽
function refreshHp()
    local max_hp = FindTreasureUtil.getFormationMaxHp()
    local map_db = _map_db
...
function refreshArrows()
    ...
    local row, col =  FindTreasureUtil.getMapPosition(_player_index)
    ...
    for k, v in pairs(direction_infos) do
        ...
        local index = FindTreasureUtil.getIndex(col + direction_info[1], row + direction_info[2])
        ...
        if map_block ~= nil then
            ...
            if old_event_info == nil or event_info.display ~= old_event_info.display then
                ...
                if event_info.display ~= 0 then
                    ...
                    -- test
                    -- if event_info.display == 2 then
                        --map_sprite:setScale(0.9)
                    -- end
                    --[[
                    if event_info.display == 2 then
                        local black = CCLayerSprite:layerSpriteWithName(CCString:create("images/treasure/effect/yanwu/yanwu"), -1,CCString:create(""));
                        map_sprite:addChild(black)
                        black:setAnchorPoint(ccp(0.5, 0.5))
                        black:setPosition(ccp(map_sprite:getContentSize().width * 0.5, map_sprite:getContentSize().height * 0.5))
                    end
                    --]]
                    ...
                    --[[ test
                    local label = CCLabelTTF:create(tostring(event_info.eid), g_sFontName, 21)
                    map_sprite:addChild(label)
...
function callbackForge()
    require "script/audio/AudioUtil"
	AudioUtil.playEffect("audio/effect/zhujiemian.mp3")
    require "script/ui/forge/ForgeLayer"
    local layer = ForgeLayer.createForgeLayer()
    MainScene.changeLayer(layer,"ForgeLayer")
end
...
function retsetMap(isConfirm, _argsCB)
    ...
    RequestCenter.dragonReset(handleResetMap, nil)
end
...
function lookFormationCallback()
    require "script/audio/AudioUtil"
	AudioUtil.playEffect("audio/effect/zhujiemian.mp3")
    RequestCenter.dragonGetUserBf(handleLookFormation, nil)
end

function handleLookFormation(cbFlag, dictData, bRet)
    handleFormation(cbFlag, dictData, bRet)
    FindFormationLayer.show(FindTreasureUtil.getFormationInfo())
end
...
function buyHp(isConfirm, _argsCB)
    ...
    RequestCenter.dragonBuyHp(handleBuyHp, args)
end

-- 买血成功
function handleBuyHp(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    local map_db = _map_db
    UserModel.addGoldNumber(-_buy_hp_gold_count)
    _map_info.hppool = tonumber(dictData.ret)
    _map_info.buyhpnum = _map_info.buyhpnum + 1
    refreshHp()
    if _map_info.hppool == 0 then
        SingleTip.showTip(string.format(GetLocalizeStringBy("key_8132"), map_db.hpPay[2]))
        FindTreasureUtil.setFormationHp(map_db.hpPay[2] / 100 * FindTreasureUtil.getFormationMaxHp())
    else
...
function handleAutoMove(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    if dictData.ret == "false" then
        print("路径没有通过服务器的验证")
        return
    end
    local path = {}
    for i = 1, #_path do
        local point = _path[i]
        local index = FindTreasureUtil.getIndex(point.x, point.y)
...
function callbackBlock(tag, menu_item)
	...
	AudioUtil.playEffect("audio/effect/zhujiemian.mp3")
    local index = tag
    local direction = FindTreasureUtil.getRelativePosition(index, _player_index)
        ...
        if _path ~= nil then
            if _map_info.act > 0 then
                ...
                for i = #_path - 1, 1, -1 do
                    local point = _path[i]
                    local index = FindTreasureUtil.getIndex(point.x, point.y)
                    args:addObject(CCInteger:create(index - 1))
                end
                --[[
                 local data = {
                 err = "ok",
                 ...
                 handleAutoMove(nil, data, nil)
                 --]]
                local args2 = CCArray:create()
                args2:addObject(args)
                _next_player_index = index
                RequestCenter.dragonAutoMove(handleAutoMove, args2)
            else
                SingleTip.showTip("行动力不足")
            end
        end
        return
    end
    ...
    if _map_info.map[index].display == 0 then
        if checkMove() == true then
            _next_player_index = index
            local args = Network.argsHandler(index - 1)
            RequestCenter.dragonMove(handleMove, args)
        end
...
function checkMove()
    ...
    if _map_info.act == 0 then
        ...
        could_move = false
    elseif FindTreasureUtil.getFormationHp() <= 0 then
...
-- 检察是否已经到终点
function checkEnd()
    local map_info  = _map_info.map[ _player_index]
    if map_info.eid == 18000 then
        ...
        local tip = CCLabelTTF:create(string.format(GetLocalizeStringBy("key_8137"), _map_info.point), g_sFontPangWa, 21)
        tip_bg:addChild(tip)
        tip:setAnchorPoint(ccp(0.5, 0.5))
        tip:setPosition(ccp(tip_bg:getContentSize().width * 0.5, 290))
        --tip:setDimensions(CCSizeMake(400, 200))
        tip:setColor(ccc3(0xff, 0xf6, 0x00))
        --tip:setVerticalAlignment(kCCVerticalTextAlignmentCenter)
        
        local tip2 = {}
        tip2[1] = CCLabelTTF:create(tostring(_map_info.point), g_sFontPangWa, 21)
        tip2[1]:setColor(ccc3(0x00, 0xff, 0x18))
        tip2[2] = CCLabelTTF:create(GetLocalizeStringBy("key_8138"), g_sFontPangWa, 21)
        tip2[2]:setColor(ccc3(0xff, 0xf6, 0x00))
        local tip2_node = BaseUI.createHorizontalNode(tip2)
        tip_bg:addChild(tip2_node)
        tip2_node:setAnchorPoint(ccp(0.5, 0.5))
        tip2_node:setPosition(ccp(tip_bg:getContentSize().width * 0.5, tip:getPositionY() - 25))
        ...
        if move_data.other.drop ~= nil then
            local title = CCRenderLabel:create("通关奖励", g_sFontPangWa, 23, 1, ccc3(0x00,0x00,0x00), type_shadow)
            ...
            --[[
            move_data.other = {}
            move_data.other.drop = {}
            move_data.other.drop.item = {
                ["60204"] = "9"
            }--]]
            local item_id = nil
        ...
        menu:setTouchPriority(_touch_priority - 41)
       if _map_info.free_reset_num > 0 then
            text = GetLocalizeStringBy("key_8104")
        else
            text = GetLocalizeStringBy("key_8105")
...
function onEndLayerEvent(event_type)
    if event_type == "enter" then
        _end_layer:registerScriptTouchHandler(onTouchesHandler, false, _touch_priority - 40, true)
...
function moveEnd()
    ...
    if event_info.eid == 18000 then
        checkEnd()
    elseif event_info.status[1] == "0" and event_info.status[2] == "0" then
        if event_info.eid ~= 0 then
        ...
        if event_db.exploreType ~= 0 then
            ...
            elseif event_db.exploreConditions[1] == 11 then
                 FindTreasureUtil.bomb(_player_index)
            end
    ...
    FindTreasureUtil.setPlayerIndex(_player_index)
    refreshBlocks()
    refreshMapSprites()
    refreshAct()
    refreshFloor()
...
function checkTreasure(index)
    ...
    local event_info = _map_info.map[_player_index]
    print_t(event_info)
    if event_info.status[4] == "1" or event_info.status[2] == "1" then
        return
    end
...
-- 拉取地图信息的网络回调
function handleGetMap(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    print("getmap====")
    print_t(dictData)
    print_t(dictData.ret.movedata)
    _map_info = FindTreasureUtil.parseMapInfo(dictData.ret)
    _old_map_info = nil
    refreshMap()
    refreshAct()
    refreshFloor()
...
-- 重置的网络回调
function handleResetMap(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    local map_db = _map_db
    if _map_info.free_reset_num > 0 then
        _map_info.free_reset_num = _map_info.free_reset_num - 1
    else
        _map_info.resetnum = _map_info.resetnum + 1
    end
    ...
    if _map_info.resetnum > 0 then
        UserModel.addGoldNumber(-map_db.resetPay)
    end
...
-- 移动的网络回调
function handleMove(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    dictData.ret = dictData.ret or {}
    if dictData.ret.point == nil then
        local event_info = _map_info.map[_next_player_index]
        local event_db = parseDB(DB_Explore_long_event.getDataById(event_info.eid))
        if event_db.exploreConditions[1] == 10 and event_info.status[1] ~= "1" and event_info.status[2] ~= "1" then
            _map_info.point = _map_info.point + event_db.integralReward[2]
        end
    end
    _map_info.hasmove = "1"
    FindTreasureUtil.handleMove(dictData.ret)
...
function moveTo(path)
    ...
    for i = #path - 1, 1, -1 do
        ...
        local curMoveEnd = function()
            local index = nil
            if _path ~= nil then
                index = FindTreasureUtil.getIndex(_path[i].x, _path[i].y)
            else
                index = _next_player_index
            end
            FindTreasureUtil.setPlayerIndex(index)
            refreshBlocks()
            refreshMapSprites()
            _player_arrows:setVisible(false)
        end
...
-- 拉取阵型的网络回调
function handleFormation(cbFlag, dictData, bRet)
    if dictData.err ~= "ok" then
        return
    end
    FindTreasureUtil.setFormationInfo(dictData.ret)
    if _reset_data ~= nil then
        handleGetMap(nil, _reset_data.dictData, nil)
        _reset_data = nil
    end
end
