...
local function confirmMenuAction( tag, itemBtn )
	...
	closeAction()
	-- 回调方法
	ForgeViewLayer.choosedEquipCallFun()
end
...
local function initChooseViewLayer( ... )
	...
	_curData = {}
	for k,v in pairs(bagInfo.arm) do
		if(tonumber(v.item_template_id) == _needItemId )then
			table.insert(_curData, v)
		end
	end
	print("_curData ==")
...
--- 
-- @des    :创建装备选择界面
-- @param  :p_needItemId  需要选择的配方id
-- @return :返回装备选择界面
function showChooseViewLayer( p_needItemId )
	init()
