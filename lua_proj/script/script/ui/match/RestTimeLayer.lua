...
function createMatchUserList( userInfo )
	...
	for k,v in pairs(userInfo) do
	    ...
	    local icon = createMatchUserInfo(des_sprite, font_color, fileName,sprite1,sprite2, v.uname, v.level, v.uid, card_bg, v.guild_name)
...
-- m_lv:玩家等级
-- m_uid:玩家uid
function createMatchUserInfo( m_des, m_color, fileName, cradIcon_n, cradIcon_h, m_userName, m_lv, m_uid, card_bg, guildName)
	local parent = CCNode:create()
	parent:setContentSize(CCSizeMake(168,318))
	-- 数字
	m_des:setAnchorPoint(ccp(0.5,0.5))
	m_des:setPosition(ccp(parent:getContentSize().width*0.5,parent:getContentSize().height-40))
	parent:addChild(m_des,3)
    -- 名
    local ming = CCSprite:create("images/match/ming.png")
    ming:setAnchorPoint(ccp(0,1))
    ming:setPosition(ccp(parent:getContentSize().width*0.5+25,parent:getContentSize().height-30))
