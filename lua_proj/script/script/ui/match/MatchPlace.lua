...
function createMatchUserList( userInfo )
	...
	for k,v in pairs(userInfo) do
	    ...
	    local icon = createMatchUserInfo(desc, font_color, fileName,sprite1,sprite2, v.uname, v.level, v.uid, v.guild_name)
...
function fnCardItemCallFun( tag, itemBtn )
    ...
    if( UserModel.getStaminaNumber()-2  < 0 )then
        ...
        StaminaAlertTip.showTip( upDateStamina, MatchLayer.refreshMatchGold )
        return
    end
    -- 判断武将满了
                        ...
                        for i,j in pairs(v) do
                            if(i == "rob")then
                                ...
                                if(MatchLayer._silverLabel ~= nil)then
                                    MatchLayer._silverLabel:setString( UserModel.getSilverNumber() )
                                end
...
-- m_uid:玩家uid
-- guildName:玩家军团
function createMatchUserInfo( m_des, m_color, fileName, cradIcon_n, cradIcon_h, m_userName, m_lv, m_uid, guildName)
	local parent = CCNode:create()
	parent:setContentSize(CCSizeMake(168,318))
	-- 可获得
	local font1 = CCRenderLabel:create( GetLocalizeStringBy("key_1573"), g_sFontPangWa, 25, 1, ccc3(0x00,0x00,0x00), type_stroke)
    font1:setAnchorPoint(ccp(0.5,1))
    font1:setColor(m_color)
    font1:setPosition(ccp(parent:getContentSize().width*0.5,parent:getContentSize().height))
    parent:addChild(font1,3)
    -- 描述
    ...
    font2:setPosition(ccp(parent:getContentSize().width*0.5,parent:getContentSize().height-28))
    parent:addChild(font2,3)
    -- 光束
    local light = CCSprite:create("images/match/" .. fileName .. ".png")
    light:setAnchorPoint(ccp(0.5,1))
    light:setPosition(ccp(parent:getContentSize().width*0.5,parent:getContentSize().height-40))
