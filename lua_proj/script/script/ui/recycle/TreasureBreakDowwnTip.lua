-- Filename: TreasureBreakDowwnTip.lua
-- Author: zhang zihang
-- Date: 2014-2-14
-- Purpose: 该文件用于: 炼化宝物提示

module ("TreasureBreakDowwnTip", package.seeall)
...
function showLayer( itemInfo, callBackFn,touchProperty, zOrder )
    ...
    local content4 = CCLabelTTF:create( GetLocalizeStringBy("key_2977"), g_sFontName ,24)
    ...
    contentNew2:setColor(HeroPublicLua.getCCColorByStarLevel(tonumber(itemInfo[1].itemDesc.quality)))
    local contentNew3 = CCLabelTTF:create(GetLocalizeStringBy("key_2191"), g_sFontName ,24)
    contentNew3:setColor(ccc3(0x78,0x25,0x00))

    local contentNew = BaseUI.createHorizontalNode({contentNew1, contentNew2,contentNew3})
