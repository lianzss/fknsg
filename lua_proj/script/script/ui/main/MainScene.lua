...
local function fn_onNetwork(cbFlag, dictData, bRet)

    if not bRet then
        return
    end
   -- 处理获取所有英雄回调
    if (cbFlag == _netFlagGetAllHeroes) then
...
--	layer:addChild(_avatarAttrBg)
    ...
    _fight_value:setPosition(476, 44)
...
function updateAvatarInfo()
    ...
    -- 银两实际数据
    _silver_num:setString( "" .. tonumber(userInfo.silver_num))
    -- 金币实际数据
...
function fnUpdateFightValue( ... )
    print("fnUpdateFightValue")
    if not _avatarAttrBg then
        return
    end
 --   local fight_value = tonumber(UserModel.getFightForceValue())
    -- 战斗力
    require "script/model/hero/HeroModel"
...
function enter (...)
    ...
    -- 主界面背景，按屏幕尺寸进行伸缩
    local mainBg = CCSprite:create(IMG_PATH.."main_bg.jpg")
    ...
    --加入鸟动画
    local birdsSprite = CCLayerSprite:layerSpriteWithName(CCString:create("images/base/effect/main/niao"), -1,CCString:create(""));
    ...
    --     _netFlagGetAllHeroes = RequestCenter.hero_getAllHeroes(fn_onNetwork)
    -- end
    -- ---------- 开始拉数据 -------
...
--    onRunningLayer:retain()
    ...
    scene:addChild(onRunningLayer, 999, 10002)
...
--            CCDirector:sharedDirector():endToLua()
    ...
    scene:registerScriptHandler(function ( eventType )
        if(eventType == "enter") then
            -- if(Platform.getPlName() == "appstore" and string.checkScriptVersion(NSBundleInfo:getAppVersion(), "4.1.0") >= 0 and NSBundleInfo:getSysVersion()~=nil and tonumber(NSBundleInfo:getSysVersion()) >= 8.0) then
            --     -- 申请步数的权限
            --     BTHealthKit:shareBTHealthKit()
            -- end
            ...
            print(GetLocalizeStringBy("key_2937"))
            -- 更新体力耐力数据
            updateInfoData()
            updateAvatarInfo()
            fnUpdateFightValue()
...
if(newLayer==nil)then
    ...
    --回调
    if(nil~=onRunningLayerCallbackFunc)then
        pcall(onRunningLayerCallbackFunc)
    end
...
function loadOther( ... )
    ...
    print(GetLocalizeStringBy("key_3214"))
    print_t(serverInfo)
    require "script/Platform"
