...
function createCenterUi( ... )
	...
	local norTitle  =  CCRenderLabel:create("每日膜拜", g_sFontPangWa, 35, 1, ccc3( 0x00, 0x00, 0x00), type_shadow)
	...
	local higTitle  =  CCRenderLabel:create("每日膜拜", g_sFontPangWa, 35, 1, ccc3( 0x00, 0x00, 0x00), type_shadow)
	...
	local grayTitle  =  CCRenderLabel:create("每日膜拜", g_sFontPangWa, 35, 1, ccc3( 0x00, 0x00, 0x00), type_shadow)
...
function crateRole( ... )
	...
	for i,v in ipairs({2, 1, 3}) do
		if(championInfo[v].htid ~= nil) then
		    ...
		    animSprite:setPosition(ccpsprite(0.5,0.2,kingHat))
		    kingHat:addChild(animSprite)
		    if(i == 1) then
		    	animSprite:setPosition(ccpsprite(0.5,0.15,kingHat))
		    end
