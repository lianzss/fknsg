...
function createTopBar( ... )
	...
	-- 银币
	silverLabel = CCLabelTTF:create(UserModel.getSilverNumber(), g_sFontName, 20)
...
function createUI( ... )
	...
	print("teacherInfo")
	print_t(teacherInfo)
	local teacherSprite = StarSprite.createStarSprite(teacherInfo.star_tid)
	local tempTable = {
		[1] = {ccp(170,292), userSprite, UserModel.getUserName(), HeroUtil.getHeroLocalInfoByHtid(UserModel.getAvatarHtid()).star_lv},
		[2] = {ccp(470,292), teacherSprite, teacherInfo.starTemplate.name, teacherInfo.starTemplate.quality},
	}
	for i=1,2 do
		...
		generalSprite:setPosition(stageSprite:getContentSize().width/2,35)
	...
	--师徒关系
	local affinityTable = {
		[1] = {GetLocalizeStringBy("zz_57"), ccp(46,20)},
		[2] = {"00", ccp(180,20)},
	}
	for i = 1,2 do
		local label = CCRenderLabel:create(affinityTable[i][1], g_sFontName, 21, 2, ccc3(0x00,0x00,0x00), type_shadow)
		label:setColor(ccc3(0x00,0xff,0x18))
		label:setAnchorPoint(ccp(0,0.5))
		label:setPosition(affinityTable[i][2])
		affinityLevelBg:addChild(label)
		if i == 2 then
			_affinityLevelLabel = label
		end
	end

	--亲密值图标
	local topAffinityIcon = CCSprite:create("images/replaceskill/affinity.png")
	topAffinityIcon:setAnchorPoint(ccp(0,0.5))
	topAffinityIcon:setPosition(136,19)
	affinityLevelBg:addChild(topAffinityIcon)
