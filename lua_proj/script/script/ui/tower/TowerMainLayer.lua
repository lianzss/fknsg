...
-- 通关条件
	...
	_passConditionLabel:setPosition(ccp(120*g_fScaleX, _layerSize.height-130*g_fScaleY))
...
function createMainUI()
	...
	preBoxMenuItem:setPosition(ccp(_layerSize.width-100*g_fScaleX, _layerSize.height-(_topBg:getContentSize().height+10)*g_fScaleX))
	...
	rankMenuItem:setPosition(ccp(_layerSize.width-10*g_fScaleX, _layerSize.height-_topBg:getContentSize().height*g_fScaleX))
	...
	passConditionTittle:setPosition(ccp(20*g_fScaleX, _layerSize.height-130*g_fScaleY))
	...
	timesTittle:setPosition(ccp(20*g_fScaleX, _layerSize.height-160*g_fScaleY))
	...
	for i=1, TowerUtil.getMaxFailedTimes() do
		...
		loveSprite:setPosition(ccp(120*g_fScaleX + 35*(i-1)*g_fScaleX, _layerSize.height-165*g_fScaleY))
		...
		grayLoveSprite:setPosition(ccp(120*g_fScaleX + 35*(i-1)*g_fScaleX, _layerSize.height-165*g_fScaleY))
...
----------- 通关奖励
	...
	passAwardSprite:setPosition(ccp(0, _layerSize.height-210*g_fScaleY))
...
function showSecret()
	...
	if(_secretNum ~= 0)then
		...
		secretItem:setPosition(ccp(20*g_fScaleX,_layerSize.height-480*g_fScaleY))
...
function doSecretBattleCallback(  newData, isVictory, extra_reward )
	...
	if(isPassed == true)then
		...
		-- elseif(power>0 and power<5)then
		-- 	UserModel.addEnergyValue(-power)
		-- else
...
function createFloorReward()
	...
	if(_curFloorDesc.isShow ~= nil and _curFloorDesc.isShow == 1 )then
		...
		floorRewardBtn:setPosition(ccp(150*g_fElementScaleRatio, _layerSize.height-230*g_fScaleY))
