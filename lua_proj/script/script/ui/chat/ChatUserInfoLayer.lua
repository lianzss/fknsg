...
function talkInPm()
    if tostring(UserModel.getUserUid()) == m_uid then
        SingleTip.showTip("不能和自己私聊")
        return
    end
    require "script/ui/chat/ChatPmLayer"
    ChatPmLayer.setTargetName(m_uname)
...
function showChatUserInfoLayer(uname,ulevel,power,img,uid,uGender,htid,dressInfo, touchPriority)
    ...
    powerLabel:setPosition(ccp(98, 25))
    ...
    local leaveMessageButton = LuaCC.create9ScaleMenuItem(image_n,image_h,CCSizeMake(170,64), "留言",ccc3(255,222,0))
    ...
    local back_btn = LuaCC.create9ScaleMenuItem(image_n,image_h,CCSizeMake(170,64),"返回",ccc3(255,222,0))
    ...
    m_ChatUserInfoLayer:registerScriptTouchHandler(cardLayerTouch,false, touchPriority + 1,true)
...
function leaveMessageCallback()
    ...
    if UserModel.getHeroLevel() >= 15 then
        ...
        SingleTip.showTip("等级达到15级以上才能留言")
    end
end
