...
function gotoRegister( ... )
	...
	local url = renewpassUrl ..  "&username=" .. username
	url 	  = url .. "&passwordOld=" .. passwordOld
	url 	  = url .. "&passwordNew=" .. password
...
function doLogin( ... )
	...
	if(username == "" or password == "")then
		require "script/ui/tip/AlertTip"
    	AlertTip.showAlert(GetLocalizeStringBy("key_1384"), nil)
		return
	end
	local url = renewpassUrl .. "&username=" .. username .. "&password=" .. password .. "&ext="
	httpClent = CCHttpRequest:open(url, kHttpGet)
    print("url",url)
 	LoadingUI.addLoadingUI()
