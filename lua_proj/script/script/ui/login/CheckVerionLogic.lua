...
if( g_debug_mode == true)then
	...
	g_checkVerion_url = "http://mapifknsg.zuiyouxi.com/phone/get3dVersion?"
end
...
function startCheckVersion(updateDelegate)
	...
	local check_version_url = g_checkVerion_url .. "&packageVersion=" .. g_publish_version .. "&scriptVersion="..g_game_version  .. Platform.getUrlParam()
...
function checkVersionCallback( res, hnd )
	...
	elseif type(versionJsonString) == "string" and string.len(versionJsonString) > 0 then
	    ...
	    if( table.isEmpty(version_info) == true )then
	    	...
	    	elseif( version_info.base and not table.isEmpty(version_info.base) and version_info.base.is_force == 1)then
	    		-- 更新底包
