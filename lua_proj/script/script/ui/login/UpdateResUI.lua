...
if(g_debug_mode == true)then
	g_download_url = "http://192.168.1.38/static/tmp/"
else
	g_download_url = "http://static1.zuiyouxi.com/sanguo/"
end
...
local _curLoginStatus 		= nil 	-- 状态，是第一次登陆还是短线重连
...
function init()
	...
	_curLoginStatus 		= nil 	-- 状态，是第一次登陆还是短线重连
end
...
local function getRightStatus()
	...
	if(Platform.getOS() == "ios")then
		if(NSBundleInfo and checkScriptVersion(NSBundleInfo:getAppVersion(), "2.0.0") >= 0 )then
			-- 如果是新版 
...
local function isCurVersionOver( ... )
	...
	if(Platform.getOS() == "ios")then
		if(NSBundleInfo and checkScriptVersion(NSBundleInfo:getAppVersion(), "2.0.0") >= 0 )then
			...
			if(sigleDownloadSize>= _curSigleSizeSvr)then
				isOver = true
			else
				isOver = false
			end
...
function createConfirmTipLayer()
    ...
    tRichTextInfo[2] = {content=displayString,ntype="label",font=g_sFontPangWa,color=ccc3(1,0x75,0x0c),fontSize=24}
...
-- 进入
function showUI(pVersionInfo, loginStatus)
	init()
	-- 先释放内存资源，防止大数据解压内存不足
