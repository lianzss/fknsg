...
-- 体力
    ...
    -- 体力值信息
    _energy = CCLabelTTF:create (g_maxEnergyNum.."/"..g_maxEnergyNum, g_sFontName, 18)
...
function updateEnergyValueUI()
    ...
    _energy:setString(energyValue.."/"..g_maxEnergyNum)
    local width = _nEnergyProgressOriWidth
    if energyValue < g_maxEnergyNum then
        width = math.floor(energyValue*_nEnergyProgressOriWidth/g_maxEnergyNum)
    end
...
function createFortsLayout( fortsData, openTargetStongholdId)
    ...
    if(file_exists("images/copy/ncopy/overallimage/" .. copy.background))then
        layoutSprite = CCSprite:create("images/copy/ncopy/overallimage/" .. copy.background)
    else
        layoutSprite = CCSprite:create("images/copy/ncopy/overallimage/" .. string.sub(copy.background,1,string.len(copy.background)-4) .. ".webp")
    end
		...
		for k,fortInfo in pairs(copy.models.normal) do
			...
			if ( baseid == fortInfo.looks.look.armyID) then
				...
				if( tonumber(baseid) == lastBaseId )then
					-- lastBaseId = tonumber (baseid) 
					--added by 张梓航
					if fortDesc.progressStatus <= "2" then
