...
function addAtkAction( ... )
	...
	BuyCopyAtkLayer.showLayer( 1, DataCache.getEliteCopyData().buy_atk_num ,refreshCanDefeatNum )
end
...
function goldTreeCallback( cbFlag, dictData, bRet )
	if(dictData.err == "ok")then
		...
		BattleLayer.showBattleWithString(dictData.ret.fightRet, nil, AfterTreeBoss.creteAfterTreeBossLayer(hurt_num, add_siliver, nil), "ducheng.jpg", nil, strongholdInfo.army_ids_simple,PlayerMoneyTreeLayer.createMoneyTreeLayer(strongholdInfo.army_ids_simple), true)
	...
	if( tag ~= curCopyTag) then
		...
		elseif(tag == Normal_Copy_Tag) then
			...
			if (table.isEmpty(data)) then
				RequestCenter.getNormalCopyList(CopyLayer.getNormalCopyCallback)
			end
...
--[[
	...
	@return void
--]]
function getNormalCopyCallback( cbFlag, dictData, bRet )
