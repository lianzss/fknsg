...
local function getNormalCopyList( obj )
	...
	if(table.isEmpty(copyInfos))then
		RequestCenter.getNormalCopyList(getNormalCopyCallback)
	else
		local maxCopyId = 0
	...
	if(not table.isEmpty(extra_reward))then
		...
		if( (not table.isEmpty(extra_reward.item)) and (not table.isEmpty(extra_reward.item)))then
			...
			elseif(item_tmpl_id >= 500001 and item_tmpl_id <= 600000) then
				...
				if ItemUtil.isExpTreasById(item_tmpl_id,nil) then
					require "scriptui/item/TreasInfoLayer"
					showLayer = TreasInfoLayer.createLayer( item_tmpl_id,  nil, false, false, false, nil, nil, nil, -5001, 2)
				else
					require "script/ui/item/NewTreasInfoLayer"
					showLayer = NewTreasInfoLayer.createLayer( item_tmpl_id,  nil, false, false, false, nil, nil, nil, -5001, 2)
				end
				local runningScene = CCDirector:sharedDirector():getRunningScene()
				runningScene:addChild(showLayer, 10001)
			else
