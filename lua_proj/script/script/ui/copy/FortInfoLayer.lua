...
function fight10Action( tag, itemBtn )
	...
	if ( selectedHardLevel == 1 ) then
		_bodyCost = _strongHoldInfo.cost_energy_simple * _defeat_num
	elseif( selectedHardLevel == 2 ) then
		_bodyCost = _strongHoldInfo.cost_energy_normal * _defeat_num

	elseif( selectedHardLevel == 3 ) then
		_bodyCost = _strongHoldInfo.cost_energy_hard * _defeat_num
	end
