...
function initWithHeroHID(hero_hid)
    ...
    if couldComprehendHero(_hero_data) == false then
        SingleTip.showTip("该武将暂无武将列传，无法领悟觉醒能力")
        return false
    end
...
function show(hero_hid, callback)
    if initWithHeroHID(hero_hid) == false then
        return
    end
    if not DataCache.getSwitchNodeState(ksHeroBiography) then
        return
    end
    
    if not StarUtil.isHeroCopyPassed(_hero_db.id, 1) then
        showGotoStarLayer(_hero_db.id, callback)
        return
    end
    local awaken_copy_db = string.split(_hero_db.hero_copy_id, ",")
    local need_advance_level = tonumber(string.split(awaken_copy_db[1], "|")[3])
    if tonumber(_hero_data.evolve_level) < need_advance_level then
...
function loadPlayerInfo()
    ...
    -- 银币实际数据
    _silver_count_labelvalue = CCLabelTTF:create(user_info.silver_num, g_sFontName, 18)
...
function loadComprehend()
    ...
    local awaken_radio_items_text = {"key_8080", "key_8081"}
	...
	local card = HeroPublicCC.createSpriteCardShow(_hero_db.id)
    ...
    --[[ test
    local layer = CCLayerColor:create(ccc4(100, 0, 0, 100))
    select_btn:addChild(layer)
    layer:setContentSize(select_btn:getContentSize())
    --]]
    ...
    refreshTalentInfo()
    local book = CCSprite:create("images/biography/book.png")
    _attr:addChild(book, 10)
    book:setPosition(ccp(413, _attr:getContentSize().height - 350))
    
    -- 提示信息
    --[[
    local tip_lable = createTip()
    _comprehend:addChild(tip_lable)
    tip_lable:setAnchorPoint(ccp(0.5, 1))
    local tip_lable_y =  items_list:getPositionY() - items_list:getContentSize().height - 15
    tip_lable:setPosition(ccp(comprehend_size.width * 0.5, tip_lable_y))
    --]]
end
...
function createAwakenCell(awaken_info)
    ...
    if awaken_info.name ~= nil then
        ...
        --awaken_name_lable:setColor(ccc3(0x00, 0xe4, 0xff))
...
function refreshTalentInfo()
    --[[
    if _talent_info_node ~= nil then
        _talent_info_node:removeFromParentAndCleanup(true)
    end
    _talent_info_node = createTalentInfo()
    _attr:addChild(_talent_info_node)
    _talent_info_node:setAnchorPoint(ccp(0, 0.5))
    _talent_info_node:setPosition(ccp(422, _attr:getContentSize().height - 15))
    --]]
    ...
    if awaken_id ~= nil then
        ...
        local awaken_copy_db = string.split(_hero_db.hero_copy_id, ",")
        local need_advance_level = tonumber(string.split(awaken_copy_db[_current_awaken_index], "|")[3])
        if StarUtil.isHeroCopyPassed(_hero_db.id, _current_awaken_index) == false or tonumber(_hero_data.evolve_level) < need_advance_level then
            awaken_info.des = GetLocalizeStringBy("key_8018") .. tostring(need_advance_level) ..
                            GetLocalizeStringBy(copy_level_tip[_current_awaken_index])
...
function loadMenu()
    ...
    _not_change_btn:setScale(MainScene.elementScale * 0.98)
   ...
   _change_btn:setScale(MainScene.elementScale * 0.98)
    ...
    _comprehend_btn:setScale(MainScene.elementScale * 0.98)
    _comprehend_btn:setAnchorPoint(ccp(0.5, 0))
    _comprehend_btn:setPosition(right_point)
	_comprehend_btn:registerScriptTapHandler(callbackComprehend)
    
    -- 激活
    _activate_btn = LuaCC.create9ScaleMenuItem("images/common/btn/purple01_n.png", "images/common/btn/purple01_h.png", btn_size, "觉醒激活", text_color, 35, g_sFontPangWa,1, ccc3(0x00, 0x00, 0x00))
	_menu:addChild(_activate_btn)
    _activate_btn:setScale(MainScene.elementScale * 0.98)
...
function activateCallback()
    ...
    local need_advance_level = tonumber(string.split(awaken_copy_db[_current_awaken_index], "|")[3])
    if StarUtil.isHeroCopyPassed(_hero_db.id, _current_awaken_index) == false or tonumber(_hero_data.evolve_level) < need_advance_level then
            local tip = GetLocalizeStringBy("key_8018") .. tostring(need_advance_level) ..
                            GetLocalizeStringBy(copy_level_tip[_current_awaken_index])
...
function handleActivate(cbFlag, dictData, bRet)
   ...
   _hero_data.talent.sealed = _hero_data.talent.sealed or {}
    local richInfo = {}
    richInfo.elements = {}
    local texts = {["1"] = GetLocalizeStringBy("key_8336"), ["2"] = GetLocalizeStringBy("key_8337")}
    ...
    --[[
    for awaken_index, awaken_id in ipairs(_hero_data.talent.sealed) do
        if awaken_id ~= "0" then
            local awaken_copy_db = string.split(_hero_db.hero_copy_id, ",")
            local need_advance_level = tonumber(string.split(awaken_copy_db[tonumber(awaken_index)], "|")[3])
            if StarUtil.isHeroCopyPassed(_hero_db.id, _current_awaken_index) == true and tonumber(_hero_data.evolve_level) >= need_advance_level then
                _hero_data.talent.sealed[awaken_index] = nil
            end
        end
    end
    --]]
...
function comprehend(p_confirmed, arg)
    if p_confirmed == true then
        local args = Network.argsHandler(_hero_data.hid, _current_awaken_index, _current_cost_index)
...
function handleComprehendTalent(cbFlag, dictData, bRet)
    ...
    -- 扣除物品
    _current_item_count = _current_item_count - cost_info.item_count
...
function createCostInfoNode()
    ...
    radio_menu:setPosition(ccp(63, bg_ng:getContentSize().height * 0.5 - 5))
    ...
    for i = 1, n do
        ...
        local y = bg_content_size.height - (i  - 0.5) * 53 - 4
        tip:setPosition(ccp(95, y))
        ...
        local cost_cell_position_x = {168, 266, 393}
        ...
        for i = 1, #cost_cell_datas do
            ...
            if cost_cell_data.count ~= 0 then
                ...
                cost_cell:setPosition(ccp(cost_cell_position_x[current_index], y))
        ...
        if i ~= n then
            ...
            line:setScaleX(4.5)
            line:setPosition(bg_content_size.width * 0.5, y - 19)
            line:setAnchorPoint(ccp(0.5, 0.5))
            bg_ng:addChild(line)
        end
    end
   ...
   -- _items_title:setPosition(ccp(35, attr:getPositionY() - attr:getContentSize().height - 28))
...
function couldComprehendHero(hero)
    ...
    return false
    --[[
    local condition_2 = StarUtil.isHasHeroCopyBy(hero.htid)
    local condition_4 = StarUtil.isHeroCopyPassed(hero.htid)
    local condition_3 = tonumber(hero.evolve_level) >= tonumber(_hero_db.refreshNeedAdvance)
    local ret = condition_1 and condition_2 and condition_3 and condition_4
    --]]
end
