...
function refreshScrollView()
    ...
    layer:setContentSize(CCSizeMake(600, 380))
    ...
    local title_text = {"key_8080", "key_8081"}
    for i = 1, 2 do
        ...
        if awaken_id ~= nil then
            ...
            local des = nil
            local des_color = nil
            local need_advance_level = _hero_db.hero_copy_id[i][3]
            if StarUtil.isHeroCopyPassed(_hero_db.id, i) == false or tonumber(_hero_data.evolve_level) < need_advance_level then
                des = GetLocalizeStringBy("key_8018") .. tostring(need_advance_level) .. GetLocalizeStringBy(copy_level_tip[i])
        ...
        local arrows = CCSprite:create("images/common/right.png")
        ...
        if _selected_hero_data ~= nil then
            ...
            if selected_awaken_id ~= nil then
                ...
                local des = nil
                local des_color = nil
                local need_advance_level = _selected_hero_db.hero_copy_id[i][3]
                if StarUtil.isHeroCopyPassed(_selected_hero_db.id, i) == false or tonumber(_selected_hero_data.evolve_level) < need_advance_level then
                    des = GetLocalizeStringBy("key_8018") .. tostring(need_advance_level) .. GetLocalizeStringBy(copy_level_tip[i])
                    des_color = ccc3(0xe4, 0x00, 0x00)
                else
                    des = GetLocalizeStringBy("key_8007")
...
function couldComprehendHero(hero)
    ...
    if hero_db.hero_copy_id ~= nil and #hero_db.hero_copy_id >= 1 then
        if StarUtil.isHeroCopyPassed(hero_db.id, 1) then
            if tonumber(hero.evolve_level) >= hero_db.hero_copy_id[1][3] then
                return true
            end
        end
    end
...
function selectAwakenCallback(tag, menu_item)
    ...
    local need_advance_level = _selected_hero_db.hero_copy_id[tag][3]
    if tonumber(_selected_hero_data.evolve_level) < need_advance_level then
        SingleTip.showTip(GetLocalizeStringBy("key_8033") .. tostring(need_advance_level) .. GetLocalizeStringBy("key_8034"))
        return
    end
    
    if StarUtil.isHeroCopyPassed(_hero_db.id, tag) == false then
        ComprehendLayer.showGotoStarLayer(_hero_db.id)
        return
    else
        if _hero_data.talent ~= nil and _hero_data.talent.confirmed ~= nil then
            if _hero_data.talent.confirmed[tostring(tag)] == nil then
                SingleTip.showTip(GetLocalizeStringBy("key_8197"))
                return
            end
        elseif _hero_data.talent == nil then
            SingleTip.showTip(GetLocalizeStringBy("key_8197"))
            return
        end
    end

    if StarUtil.isHeroCopyPassed(_selected_hero_db.id, tag) == false then
        ComprehendLayer.showGotoStarLayer(_selected_hero_db.id)
        return;
    end
...
function handleInherit(cbFlag, dictData, bRet)
    ...
    for k, v in pairs(_selected_info) do
        ...
        _hero_data.talent.confirmed[k] = nil
    end
