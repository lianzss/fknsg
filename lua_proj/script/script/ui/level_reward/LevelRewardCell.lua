...
local function receiveCallback(cbFlag, dictData, bRet )
	...
	_itemBtn:setEnabled(false)
	_itemBtn = tolua.cast(_itemBtn, "CCMenuItemSprite")
	local disableSprite = _itemBtn:getDisabledImage() ---,"CCSprite")
...
function createCell(cellValues)
	...
	normalLabel:setPosition(ccp(btnSize.width,btnSize.height))
	...
	selectLabel:setPosition(ccp(btnSize.width,btnSize.height))
	...
	disabledLabel:setPosition(ccp(disabledReceiveSprite:getContentSize().width/2,btnSize.height))
	disabledLabel:setAnchorPoint(ccp(0.5,1))
