...
function create( switchEnum )
	...
	local x = background:getContentSize().width*0.5 - (textInfo[1]:getContentSize().width + textInfo[2]:getContentSize().width + textInfo[3]:getContentSize().width)*0.5
	local y = background:getContentSize().height *  0.55
	textInfo[1]:setColor(ccc3(0xff,0xe4,0x00))
	textInfo[1]:setPosition(ccp(x, y))
	background:addChild(textInfo[1])

	
	textInfo[2]:setColor(ccc3(65, 255, 185))
	local x = textInfo[1]:getContentSize().width +  textInfo[1]:getPositionX()
	local y = textInfo[1]:getPositionY()
	textInfo[2]:setPosition(ccp(x, y))
	background:addChild(textInfo[2])

	
	textInfo[3]:setColor(ccc3(0xff,0xe4,0x00))
	local x = textInfo[2]:getContentSize().width +  textInfo[2]:getPositionX()
	local y = textInfo[2]:getPositionY()
	textInfo[3]:setPosition(ccp(x, y))
	background:addChild(textInfo[3])
...
function showNewSwitch( switchEnum )
	...
	if(isFight == true) then
		...
		-- 关闭跟新手引导冲突的 神秘商人板子
		require "script/ui/rechargeActive/MysteryMerchantDialog"
