...
local function menu_item_tap_handler(tag, item_obj)
	...
	elseif (tag >= _ksTagHeroBegin and tag <= _ksTagHeroBegin + #_arrHeroesValue) then
		...
		-- 测试代码
		print(GetLocalizeStringBy("key_2957"))
		HeroInfoLayer.createLayer(_arrHeroesValue[tag-_ksTagHeroBegin], {isPanel=true},nil,nil,true, refreshTableView)
		--MainScene.changeLayer(HeroInfoLayer.createLayer(_arrHeroesValue[tag-_ksTagHeroBegin],nil, nil , nil ,true), "HeroInfoLayer")
...
fnCreateHeroTableView = function (layer)
	...
	print("武将列表信息,afjkjfaj")
	print_t(_arrHeroesValue)
	...
	local handler = LuaEventHandler:create(function(fn, table, a1, a2)
		...
		elseif (fn == "cellAtIndex") then
			...
			-- end
			r = HeroLayerCell.createCell(_arrHeroesValue[length-a1])
			value.ccCellObj = r
...
function getArrHeroesValue( ... )
	...
	for i=1, nArrLen do
		...
		for j=1, #value.menu_items do
			print("当前武将进阶等级",value.evolve_level)
			print("当前按钮tag",value.menu_items[1].tag)
...
function setHeroListOffSet(offset )
	...
	print('maxContentOffset:', maxContentOffset)
end
...
function getAvatarTransferButton( ... )
	...
	local item = tolua.cast(menu:getChildByTag(_ksTagMetempsychosis+1), "CCMenuItem")
...
function layerDidLoadCallfunc( ... )
	...
	print(GetLocalizeStringBy("key_2417"))
	--武将进阶新手引导
	require "script/guide/NewGuide"
	require "script/guide/GeneralUpgradeGuide"
...
function getTitleTipSprite( ... )
	if(_fuseHeroTipSprite) then
		...
		print("don't find title tip sprite")
		return nil
	end
end
