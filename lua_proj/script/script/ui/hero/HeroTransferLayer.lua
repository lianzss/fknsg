...
function fnCreateTransferEffect(pForceValues01, pForceValues02, pHeroAttr)
	...
	if _tHeroTransferedAttr then
		...
		local csCardShow = HeroPublicCC.createSpriteCardShow(_tHeroTransferedAttr.new_htid)
...
local function fnHandlerOfTransfer(tag, obj)
	...
	-- printTable("_tHeroAttr", _tHeroAttr)
	-- print("isHaveTranfer", isHaveTranfer)
...
local function fnHandlerCloseBtn(tag, item_obj)
	...
	if _tParentParam then
		-- local createLayer= _tParentParam.fnCreate(_tParentParam.reserved)
		MainScene.changeLayer(_tParentParam.fnCreate(_tParentParam.reserved), _tParentParam.sign)
		if( _tParentParam.sign == "HeroLayer") then
...
-- 创建“消耗银币”信息面板
fnCreateSilverInfoPanel = function ()
	-- 底(X69 Y15 W24 H14)
	-- 背景九宫格图
...
local function createCell(tParam)
	...
	if tParam.type == _ksTypeOfCard then
		headIcon = HeroPublicCC.getCMISHeadIconByHtid(tParam.htid)
	else
...
-- 创建武将属性面板
fnCreateHeroAttrPanel = function (tParam)
	-- 背景图(9宫格)
    ...
    require "script/model/hero/HeroModel"
 	...
 	x = x+tLabelAttrObjs[1]:getContentSize().width
	...
	tForceValue = HeroFightForce.getAllForceValues(tParam)
 	...
 	local tLabels = {
 		{text=tForceValue.magicDefend, fontsize=18, color=ccc3(0, 0, 0)},
 		{text=tForceValue.physicalDefend, vOffset=10},
		{text=tForceValue.generalAttack},
 		{text=tForceValue.life},
 		{text=tParam.level}
 	}
...
fnCreateSelectedHeroPanel = function ( ... )
	...
	-- 卡牌角色展示
	local ccSpriteCardShow = CCSprite:create("images/base/hero/body_img/"..tDB.body_img_id)
	ccSpriteCardShow:setPosition(ccp(ccSpriteBg:getContentSize().width/2, 60))
...
-- 创建进阶后下级形象面板
fnCreateHeroNextLevelPanel = function ( ... )
