...
local function fnCreateHeroAttrPanel( ... )
    ...
    -- 银币实际数据
    _cltSilverNum = CCLabelTTF:create(userInfo.silver_num, g_sFontName, 18)
...
function fnUpdateHeroAttrPanel()
	...
	-- 银两实际数据
	_cltSilverNum:setString(userInfo.silver_num)
...
function createLayer(hero, tParam)
	init()
	if hero then
		_tHeroAttr = hero
	end
...
function fnHandlerOfNetwork(cbFlag, dictData, bRet)
	...
	if cbFlag == "hero.enforceByHero" or cbFlag == "hero.enforce" then
		...
		local addedLv = tonumber(dictData.ret.level) - _tHeroAttr.level
		PublicSpecialEffects.enhanceResultEffect(addedLv)
		...
		if cbFlag == "hero.enforceByHero" then
			...
			fnAddProgressBar()
--			fnRenewHeroAttr(tonumber(dictData.ret.level))
		...
		if addedLv > 0 then
			local tNextFightForce = HeroFightForce.getAllForceValues(_tHeroAttr)
			_tHeroAttr.level = _tHeroAttr.level - addedLv
			local tFightForce = HeroFightForce.getAllForceValues(_tHeroAttr)
			_tHeroAttr.level = _tHeroAttr.level + addedLv
			...
			local arrValues = {
				tNextFightForce.life - tFightForce.life,
				tNextFightForce.generalAttack - tFightForce.generalAttack,
				tNextFightForce.physicalDefend - tFightForce.physicalDefend,
				tNextFightForce.magicDefend - tFightForce.magicDefend,
			}
			for i=1, #arrTxt do
				tArgs[i] = {}
...
function fnNetworkSendRequest( ... )
	...
	Network.rpc(fnHandlerOfNetwork, "hero.enforceByHero","hero.enforceByHero", args, true)
end
...
local function fnHandlerOfButtonsOnSoulStrengthenPanel(tag, item_obj)
	if tag == _ksTagButtonReturnOnSoul then
		...
		AudioUtil.playEffect("audio/effect/guanbi.mp3")
		require "script/ui/main/MainScene"
		if _tParentParam == nil or (type(_tParentParam)~="table" and type(_tParentParam.fnCreate) ~= "function") then
			require "script/ui/hero/HeroLayer"
			MainScene.changeLayer(HeroLayer.createLayer(), "HeroLayer")
		else
			MainScene.changeLayer(_tParentParam.fnCreate(_tParentParam.reserved), _tParentParam.sign)
		end
...
-- 判断武将是否已达上限
		...
		Network.rpc(fnHandlerOfNetwork, "hero.enforce","hero.enforce", args, true)
	end
end
...
fnCreateSelectedHeroes = function()
    ...
    bg_ng:setPosition(g_winSize.width/2, _nYPosOfHeroAttr*190/250)
...
-- 进度条九宫格背景
    ...
    require "script/model/hero/HeroModel"
 	...
 	local tFightForce = HeroFightForce.getAllForceValues(_tHeroAttr)
 	local tArgs = {}
 	tArgs = table.hcopy(_tHeroAttr, tArgs)
 	tArgs.level = tArgs.level + 1
 	local tNextFightForce = HeroFightForce.getAllForceValues(tArgs)
 	-- 必防数值
 	local magicDefend = {
 		current=tFightForce.magicDefend,
 		next=tNextFightForce.magicDefend,
 	}
 	-- 物防数值
 	local physicalDefend = {
 		current=tFightForce.physicalDefend,
 		next=tNextFightForce.physicalDefend,
 	}

 	local generalAttack = {}
 	generalAttack.current=tFightForce.generalAttack
 	generalAttack.next=tNextFightForce.generalAttack
 	-- 生命数值
 	local life = {
 		current=tFightForce.life,
 		next=tNextFightForce.life
 	}
 	local tLabels = {
...
fnCreateStrenthenCardAndAttrPanel = function (layer)
	...
	local ccSpriteCardBG = HeroPublicCC.createSpriteCardShow(_tHeroAttr.htid)
