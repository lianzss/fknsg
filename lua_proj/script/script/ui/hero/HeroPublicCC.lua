...
function createHeroHeadIcon(tParam, dressId)
	...
	local csQuality = HeroUtil.getHeroIconByHTID(tHero.htid, dressId,nil ,UserModel.getVipLevel())
	local csQualityLighted = HeroUtil.getHeroIconByHTID(tHero.htid, dressId,nil, UserModel.getVipLevel())
...
function createSpriteCardShow(htid, dressId)
...
-- 角色展示
	...
	ccSpriteCardShow:setPosition(ccSpriteCardBG:getContentSize().width/2, 62-yOffset)
...
-- in. htid: 武将的htid
-- out. 该武将的整体头像，是个CCMenuItemSprite对象
function getCMISHeadIconFullByHtid(htid)
	require "db/DB_Heroes"
	local db_hero = DB_Heroes.getDataById(htid)
	local sHeadIconImg="images/base/hero/head_icon/" .. db_hero.head_icon_id
	local sQualityBgImg="images/hero/quality/"..db_hero.star_lv .. ".png"
