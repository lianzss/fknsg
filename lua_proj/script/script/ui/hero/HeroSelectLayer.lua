...
local function createHeroSellTableView(layer)
    ...
    tableView:setTouchPriority(-390)
--	tableView:setVerticalFillOrder(kCCTableViewFillTopDown)
	local bFocus = false
	if _arrSelectedHeroes and #_arrSelectedHeroes > 0 then
		local focusHid = tonumber(_arrSelectedHeroes[1])
		for i=1, #_arrHeroesValue do
			if tonumber(_arrHeroesValue[i].hid) == focusHid then
				local nOffset = _scrollview_height - (#_arrHeroesValue-i+1)*cellSize.height
	...
	require "script/guide/NewGuide"
	if(NewGuide.guideClass ==  ksGuideClose and not bFocus) then
