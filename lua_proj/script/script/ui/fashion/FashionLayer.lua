...
-- 判断有木有时装
	...
	if(not isDress) then
		...
		iconName = HeroUtil.getHeroBodyImgByHTID(UserModel.getAvatarHtid(),dressHtid,UserModel.getUserSex())
...
-- add by DJN 2014/08/21
	...
	local rageSkill    --技能id
    --如果技能改变了
	if tonumber(ReplaceSkillData.getChangeSkillInfo()) ~= 0 then
		rageSkill = tonumber(ReplaceSkillData.getChangeSkillInfo())
	else
		--当前装备的是自己的技能
		local db_hero = DB_Heroes.getDataById(UserModel.getAvatarHtid())
		rageSkill = db_hero.rage_skill_attack
	end
	print("输出当前技能id")
	print(rageSkill)
	--当前技能的图标
	local iconSprite = ReplaceSkillLayer.createSkillIcon(rageSkill)
