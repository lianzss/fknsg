...
function getItemButton(item_temple_id )
	...
	local headSprite = ItemSprite.getItemSpriteById(item_temple_id,nil,itemDelegateAction) --HeroPublicCC.getCMISHeadIconFullByHtid(item_temple_id)
	-- headSprite:setEnabled(boolExsit)
	if(boolExsit== false) then
		headSprite= ItemSprite.getItemGraySpriteByItemId(item_temple_id)
		-- return getEnableIcon()
	end
