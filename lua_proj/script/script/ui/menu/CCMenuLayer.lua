...
function createMenuList(  )
	...
	for i =1 ,#_menuArray do 
		...
		local descLabel = CCLabelTTF:create("" .. _menuArray[i].name, g_sFontPangWa,36)
		...
		if(_menuArray[i].nameSp) then
			createSpriteTxt(_menuArray[i].nameSp ,whiteSpite)
		end
		...
		local menuItem = createCellBtn(_menuArray[i])
		...
		menu:addChild(menuItem,0, _menuArray[i].tag)
		menuItem:registerScriptTapHandler(itemCb)
	end

	print("m_isBgmOpen is : " , AudioUtil.m_isBgmOpen)
