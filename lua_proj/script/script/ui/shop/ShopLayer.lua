...
require "script/ui/shop/PropLayer"
...
function refreshTopUI( )
	print("refreshTopUI  refreshTopUI  refreshTopUI  ")
	m_powerLabel:setString(math.floor(MainScene.fnUpdateFightValue()))
	local userInfo = UserModel.getUserInfo()
	m_silverLabel:setString(userInfo.silver_num)
...
local function shopMenuAction( tag, itemBtn )
	...
	if (_curButton ~= itemBtn) then
		...
		elseif(_curButton == _propButton) then
			_curDisplayLayer = PropLayer.createLayerBySize( CCSizeMake(bgLayerSize.width, curDisplayLayerHight) )
