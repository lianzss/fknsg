...
function createHeroSprite(hero_data, f_pos)
	...
	if(hero_data >0) then
		...
		local cardIcon = BattleCardUtil.getFormationPlayerCard(hero_data, false)
	    ...
	    if HeroModel.isNecessaryHero(heroAllInfo.htid) then
	    	heroName = CCRenderLabel:create(UserModel.getUserName(),g_sFontName,21,1,ccc3(0x00,0x00,0x00),type_stroke)
	    else
