...
local function initLittleFriendOneLayer( ... )
	...
	for i=1,6 do
		...
		if(formationData[i] > 0)then
			...
			local cardIcon = BattleCardUtil.getFormationPlayerCard(formationData[i], false)
			cardIcon:setAnchorPoint(ccp(0.5, 0))
			cardIcon:setPosition(ccp(bg:getContentSize().width*0.5, 0))
			bg:addChild(cardIcon)

			-- 英雄信息
			local heroAllInfo = HeroUtil.getHeroInfoByHid(formationData[i])
			print("heroAllInfo")
			print_t(heroAllInfo)
			-- 名字
			local hero_name = nil
			if( HeroModel.isNecessaryHeroByHid(formationData[i]) )then
				...
				hero_name = heroAllInfo.localInfo.name
			end
			...
			-- 进阶数
			local evolve_font = CCRenderLabel:create("+" .. heroAllInfo.evolve_level, g_sFontPangWa, 18, 1, ccc3(0x00, 0x00, 0x00), type_stroke)
