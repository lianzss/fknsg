...
function getPetIMGById( petTId, showStatus , slotIndex)
	...
	elseif(showStatus == 3) then 
		petShowSprite= CCSprite:create("images/pet/pet/horse_dark.png")
		local lockSprite = CCSprite:create("images/formation/potential/lock.png")
		...
		if(tonumber(needLevel)<999) then
			...
			needLevelLabel:setPosition(petShowSprite:getContentSize().width*0.6,125-goldNode:getContentSize().height)
...
local function getExpandNeedMoney()
	require "script/ui/tip/AlertTip"
	require "script/ui/pet/PetData"
    require "db/DB_Pet_cost"
    local costTable = DB_Pet_cost.getDataById(1)
    local everyNum = costTable.openFenseNum
    local costTable = DB_Pet_cost.getDataById(1)
    local baseFenseTable = DB_Pet_cost.getDataById(1)
    local baseFense = tonumber(baseFenseTable.baseFenseNum)
    local baseMoney = tonumber(costTable.openFenseBaseCost)
    local needMoney = baseMoney + tonumber((PetData.getOpenBagNum()-baseFense)/everyNum)*tonumber(costTable.openFenseGrowCost)
    return needMoney
end

local function getEveryIncrease()
	require "db/DB_Pet_cost"
	local costTable = DB_Pet_cost.getDataById(1)
    local everyNum = costTable.openFenseNum
    return tonumber(everyNum)
end

function expandSuccess(cbFlag, dictData, bRet)
    if not bRet then
        return
    end
    if cbFlag == "pet.openKeeperSlot" then
    	require "script/ui/pet/PetData"
    	require "script/model/user/UserModel"
        PetData.addOpenBagNum(getEveryIncrease())
        UserModel.addGoldNumber(tonumber(-getExpandNeedMoney()))
    end
end

function sureConfirm(isConfirm)
    if isConfirm then
    	require "script/model/user/UserModel"
        if tonumber(UserModel.getGoldNumber()) >= tonumber(getExpandNeedMoney()) then
            require "script/network/Network"
            local subArg = CCArray:create()
            subArg:addObject(CCInteger:create(1))
            Network.rpc(expandSuccess, "pet.openKeeperSlot","pet.openKeeperSlot", subArg, true)
        else
...
function makeBagLarger()
    ...
    require "db/DB_Pet_cost"
    local costTable = DB_Pet_cost.getDataById(1)
    local everyNum = getEveryIncrease()
    local baseFenseTable = DB_Pet_cost.getDataById(1)
    local baseFense = tonumber(baseFenseTable.baseFenseNum)
    local baseMoney = tonumber(costTable.openFenseBaseCost)
    local needMoney = getExpandNeedMoney()
    AlertTip.showAlert(GetLocalizeStringBy("key_1523") .. tostring(everyNum) .. GetLocalizeStringBy("key_2001") .. tostring(needMoney) .. GetLocalizeStringBy("key_1491"),sureConfirm, true)
end
...
function getPetValueByInfo( arrSkill )
