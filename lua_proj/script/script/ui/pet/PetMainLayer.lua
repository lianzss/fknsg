...
function createBotom( )
    ...
    _formationNumBg:setContentSize(CCSizeMake( 167,34))
    ...
    _swallowItem= LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png",CCSizeMake(198,73),GetLocalizeStringBy("key_1921"),ccc3(0xfe, 0xdb, 0x1c),35,g_sFontPangWa,1, ccc3(0x00, 0x00, 0x00))
    _swallowItem:setAnchorPoint(ccp(0,0))
    _swallowItem:setPosition(_layerSize.width* 0.023, 5*MainScene.elementScale)--_layerSize.height*0.078)
    _swallowItem:setScale(MainScene.elementScale )
    _swallowItem:registerScriptTapHandler(swallowAction)
    menuBar:addChild(_swallowItem,1)

    _feedItem= LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png",CCSizeMake(198,73),GetLocalizeStringBy("key_1780"),ccc3(0xfe, 0xdb, 0x1c),35,g_sFontPangWa,1, ccc3(0x00, 0x00, 0x00))
    _feedItem:setAnchorPoint(ccp(0,0))
    _feedItem:setPosition(_layerSize.width*0.343,5*MainScene.elementScale)-- _layerSize.height*0.078)
    _feedItem:setScale(MainScene.elementScale )
    _feedItem:registerScriptTapHandler(feedAction)
    menuBar:addChild(_feedItem,1)

    _realizeItem= LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png",CCSizeMake(198,73),GetLocalizeStringBy("key_2875"),ccc3(0xfe, 0xdb, 0x1c),35,g_sFontPangWa,1, ccc3(0x00, 0x00, 0x00))
    _realizeItem:setAnchorPoint(ccp(0,0))
    _realizeItem:setPosition(_layerSize.width*0.664,5*MainScene.elementScale )-- _layerSize.height*0.078)
    _realizeItem:registerScriptTapHandler(realizeAction)
    _realizeItem:setScale(MainScene.elementScale)
...
function createPropertyUI( )
    ...
    skillPropertyDesc:setPosition(_petPropertyBg:getContentSize().width/2,5)
        ...
        elseif fn == "cellAtIndex" then
           ...
           for i =1, 4 do
                ...
                if(a1*4 +i<= #_skillNormal) then 
                    local index= a1*4 +i
                    --得到技能
                    local headSprite = PetUtil.getNormalSkillIcon(_skillNormal[index].id, _skillNormal[index].level , _addNormalSkillLevel , _skillNormal[index].status, _formationPetInfo[_curPetIndex].petid ,rfcAftLock )
                    ...
                    if( tonumber(_skillNormal[index].id)>0 ) then
                        ...
                        local skillProperty = PetUtil.getNormalSkill( tonumber(_skillNormal[index].id), tonumber(_skillNormal[index].level)+_addNormalSkillLevel )
...
function createSpecialIcon( )
    ...
    local height= _layerSize.height- _topBg:getContentSize().height*_topBg:getScale()- 47*MainScene.elementScale
...
function createMiddleUI( )
    ...
    _upBtn:setPosition(_layerSize.width/2, _starsBgSp:getPositionY() -(5+ _starsBgSp:getContentSize().height)*MainScene.elementScale )
    ...
    _upAlreadySp:setPosition(_layerSize.width/2,  _starsBgSp:getPositionY() - (5+ _starsBgSp:getContentSize().height)*MainScene.elementScale )
    ...
    --等级和名字
    local lvSp= CCSprite:create("images/common/lv.png")
    _lvLabel= CCLabelTTF:create( "100" ,g_sFontPangWa, 18)-- 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    ...
    nameNode:setPosition(_nameBg:getContentSize().width/2, _nameBg:getContentSize().height/2)
...
function createPetSrc(  )
    ...
    for i=1, #_formationPetInfo do
        ...
        petSprite:setPosition(ccp(_curPetSize.width*(i-0.5) , 25*MainScene.elementScale/g_fScaleX))
...
local function switchPetSrc( xOffset )
    if (math.abs(xOffset) < 20) then
        ...
        elseif(_formationPetInfo[_curPetIndex].showStatus ==3) then
            ...
            local function openSquan( isOpen)
               -- if(isOpen == true) then
                    local petid= _formationPetInfo[_curPetIndex].petid
                    local pos = _curPetIndex-1
                    PetService.openSquandSlot( pos, rfcAtfOpenSquandSlot)
                --end
            end  
...
function feedAction( )
    ...
    if(NewGuide.guideClass ==  ksGuidePet and PetGuide.stepNum == 5) then
        ...
        local button = PetMainLayer.getFirstFoodCell()
...
function addSwalPetAction( tag, item)
    ...
    local swallowLayer=  SelSwallowPetLayer.showLayer( _formationPetInfo[_curPetIndex].petid , _swallowedPetId)
    -- MainScene.changeLayer( swallowLayer,"swallowLayer" )
