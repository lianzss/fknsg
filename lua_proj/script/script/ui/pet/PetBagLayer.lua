...
function createTopUI()
    ...
    local _silverLabel = CCLabelTTF:create( UserModel.getSilverNumber() ,g_sFontName,18)
...
function expandSuccess(cbFlag, dictData, bRet)
    ...
    if cbFlag == "pet.openKeeperSlot" then
        PetData.addOpenBagNum(tonumber(everyNum))
        UserModel.addGoldNumber(tonumber(-needMoney))
...
function sureConfirm(isConfirm)
    if isConfirm then
        if tonumber(UserModel.getGoldNumber()) >= tonumber(needMoney) then
...
local function expandFunction()
    ...
    everyNum = costTable.openFenseNum
    require "db/DB_Pet_cost"
    ...
    needMoney = baseMoney + tonumber((PetData.getOpenBagNum()-baseFense)/everyNum)*tonumber(costTable.openFenseGrowCost)
