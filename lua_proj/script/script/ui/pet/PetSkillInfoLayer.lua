...
local function refreshSprite()
	...
	skillSprite = PetUtil.getSkillIcon(_skillId,tonumber(_lv+PetData.getAddSkillByTalent(_petId).addNormalSkillLevel),_status)
...
local function createContent()
	...
	local lvNum
	if tonumber(PetData.getAddSkillByTalent(_petId).addNormalSkillLevel) >= 1 then
		lvNum = CCLabelTTF:create("lv." .. _lv .. "（+" .. PetData.getAddSkillByTalent(_petId).addNormalSkillLevel .. "）",g_sFontPangWa,24)
	else
		lvNum = CCLabelTTF:create("lv." .. _lv , g_sFontPangWa,24)
	end
