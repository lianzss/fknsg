...
local function  createPetPart()
	...
	nameUpper:setPosition(ccp(nameSize.width/2,nameSize.height/2))
	...
	if normalNum > 0 then
		...
		for i = 1 , tonumber(normalNum) do
			...
			if noOver and tonumber(normalInfo[i].id) ~= 0 then
				...
				local nhead = PetUtil.getSkillIcon(normalInfo[i].id,normalInfo[i].level + addLevel,normalInfo[i].status)
