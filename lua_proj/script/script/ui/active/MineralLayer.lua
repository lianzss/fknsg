...
local function push_mineral_updatepit_callback( cbFlag, dictData, bRet )
    ...
    print("fuckupdate", updated)
    print_t(_myMineralInfo)
    print_t(pitInfo)
...
function pushMineralRobCallback(cbFlag, dictData, bRet)
    ...
    dictData.ret.rob_time = TimeUtil.getSvrTimeByOffset(0)
    if _isShowed == true then
        table.insert(_push_mineral_robs, dictData.ret)
        showMineralRobInfoNode()
    end
    table.insert(_all_push_mineral_robs, dictData.ret)
    MineralRobInfoLayer.refresh()
end

function getAllPushMineralRobs()
    return _all_push_mineral_robs
end
...
local function createTopUI()
	...
	-- 战斗力
    _fightValueLabel = CCRenderLabel:create(MainScene.fnUpdateFightValue() , g_sFontName, 23, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
...
function refreshTopUI()
	_fightValueLabel:setString(MainScene.fnUpdateFightValue())
...
function updateTime()
    ...
    for k, v in pairs(_countDownLabel) do
        ...
        elseif my_mineral_info.expireTime - BTUtil:getSvrTimeInterval() <= 0 then
            ...
            local time_str = TimeUtil.getTimeString(my_mineral_info.expireTime - BTUtil:getSvrTimeInterval())
...
function createTwoBtn()
    ...
    for i = 1, 2 do
        ...
        _twoMenuBar:addChild(mineral_btn)
...
function createRobInfoBtn()
	...
	robInfoBtn:setPosition(MainScene.getMenuPositionInTruePoint(_bgLayer:getContentSize().width*0.8, _bgLayer:getContentSize().height*0.25))
