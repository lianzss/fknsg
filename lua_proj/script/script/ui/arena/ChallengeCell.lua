...
function createCell( tCellValue )
	...
	if( tonumber(tCellValue.uid) == UserModel.getUserUid() )then
		-- 如果是主角
		fileName = ArenaLayer.IMG_PATH .. "rankings_selfcellbg.png"
	else
		fileName = ArenaLayer.IMG_PATH .. "arena_cellbg.png"
	end
	...
	local name_bg = CCScale9Sprite:create(ArenaLayer.IMG_PATH .. "heroname_bg.png", fullRect, insetRect)
	name_bg:setContentSize(CCSizeMake(302,26))
	name_bg:setAnchorPoint(ccp(0,1))
	name_bg:setPosition(ccp(20,cellBg:getContentSize().height-15))
	cellBg:addChild(name_bg)
	-- lv.
	local lv_sprite = CCSprite:create(ArenaLayer.COMMON_PATH .. "lv.png")
    ...
    lv_data:setPosition(ccp(lv_sprite:getPositionX()+lv_sprite:getContentSize().width+5,name_bg:getContentSize().height-3))
   	...
   	if(tonumber(tCellValue.luck) == 1)then
	   	local luckSprite = CCSprite:create(ArenaLayer.IMG_PATH .. "luckicon.png")
    ...
    curRankings_font:setPosition(ccp(name_bg:getPositionX()+name_bg:getContentSize().width+35,cellBg:getContentSize().height-12))
    ...
    curRankings:setPosition(ccp(curRankings_font:getPositionX()+curRankings_font:getContentSize().width+7,cellBg:getContentSize().height-12))
	   ...
	   -- 更新倒计时
		...
		local challengeItem = CCMenuItemImage:create(ArenaLayer.IMG_PATH .. "challenge_normal.png",ArenaLayer.IMG_PATH .. "challenge_select.png")
		challengeItem:setAnchorPoint(ccp(1,1))
		challengeItem:setPosition(ccp(cellBg:getContentSize().width-20, cellBg:getContentSize().height-22))
    ...
    local insetRect = CCRectMake(30, 30, 15, 10)
   	local hero_bg = CCScale9Sprite:create(ArenaLayer.COMMON_PATH .. "bg/bg_ng_attr.png", fullRect, insetRect)
   	...
   	-- 获得的奖励
   	local silverData,prestigeData = ArenaData.getAwardItem(tCellValue.position,tCellValue.level)
   	-- 获得奖励文字
    ...
    jiangli:setPosition(ccp(55,36))
   	cellBg:addChild(jiangli)
   	-- 银币
   	local silver_icon = CCSprite:create(ArenaLayer.COMMON_PATH .. "coin_silver.png")
   	silver_icon:setAnchorPoint(ccp(0,0))
   	silver_icon:setPosition(ccp(150,12))
    ...
    silver:setPosition(ccp(silver_icon:getPositionX()+silver_icon:getContentSize().width+7,36))
   	cellBg:addChild(silver)
   	-- 声望
   	local prestige_icon = CCSprite:create(ArenaLayer.COMMON_PATH .. "prestige.png")
   	prestige_icon:setAnchorPoint(ccp(0,0))
   	prestige_icon:setPosition(ccp(295,12))
    ...
    prestige:setPosition(ccp(prestige_icon:getPositionX()+prestige_icon:getContentSize().width+7,36))
   	cellBg:addChild(prestige)
   	-- -- 物品
   	-- local item_icon = CCSprite:create(ArenaLayer.IMG_PATH .. "item_icon.png")
   	-- item_icon:setAnchorPoint(ccp(0,0))
   	-- item_icon:setPosition(ccp(440,12))
   	-- cellBg:addChild(item_icon)
   	-- -- 物品数据 
   	-- local item_num = CCRenderLabel:create( GetLocalizeStringBy("key_1291") .. itemData[1].item_num , g_sFontBold, 20, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
    -- item_num:setColor(ccc3(0xff, 0xff, 0xff))
    -- item_num:setPosition(ccp(item_icon:getPositionX()+item_icon:getContentSize().width+7,36))
   	-- cellBg:addChild(item_num)
    ...
    local function createNext( atk, isNeedReload, isUp, position, flopData)
		...
		local function nextCallFun()
			...
			if(ArenaLayer.m_silverLabel ~= nil)then
				ArenaLayer.m_silverLabel:setString( UserModel.getSilverNumber() )
			end
			-- 更新菜单栏当前排名
		...
		local afterBattleLayer = AfterBattleLayer.createAfterBattleLayer( atk.appraisal, challenge_uid, enemyData, atk.force, coin, exp, flopData, afterOKcallFun, atk.fightRet )
		BattleLayer.showBattleWithString(atk.fightRet, nextCallFun, afterBattleLayer,"zhuoretudi.jpg","music11.mp3",nil,nil,nil,true)
	end
    -- 调用后端接口
	ArenaService.challenge(tag, challenge_uid, createNext)
end
