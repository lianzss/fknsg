...
function createCell( tCellValue )
	...
	-- else
		if(tonumber(tCellValue.position) < 4)then
			fileName = ArenaLayer.IMG_PATH .. tCellValue.position .. ".png"
		else
			fileName = ArenaLayer.IMG_PATH .. "rankings_cellbg.png"
		end
	-- end
    ...
    curRankings:setPosition(ccp(curRankings_font:getPositionX()+curRankings_font:getContentSize().width+7,cellBg:getContentSize().height-12))
	   ...
	   -- 更新倒计时
		...
		local squadMenuItem = CCMenuItemImage:create(ArenaLayer.COMMON_PATH .. "btn/btn_blue_n.png",ArenaLayer.COMMON_PATH .. "btn/btn_blue_h.png")
		...
		-- 阵容字体
		local squad_font = CCRenderLabel:create( GetLocalizeStringBy("key_1953") , g_sFontPangWa, 30, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
