...
function initSearchSoulLayer( ... )
    ...
    someroMenuItem:setScale(g_fScaleX)
 	-- -- 价格
 	-- shen_goldIcon = CCSprite:create("images/common/gold.png")
	-- shen_goldIcon:setAnchorPoint(ccp(0, 1))
	-- _bgLayer:addChild(shen_goldIcon)
	-- shen_goldIcon:setScale(g_fScaleX)
	-- local goldFont = CCRenderLabel:create( _needGoldNum .. GetLocalizeStringBy("key_3030"), g_sFontName, 21, 1, ccc3(0, 0, 0), type_stroke)
	...
	-- shenIcon:setAnchorPoint(ccp(0, 1))
	-- _bgLayer:addChild(shenIcon)
	-- shenIcon:setScale(g_fScaleX)
	-- -- 居中
	-- local posX = someroMenuItem:getPositionX()-(shen_goldIcon:getContentSize().width+goldFont:getContentSize().width+shenIcon:getContentSize().width)*0.5*g_fScaleX
	-- local posY = someroMenuItem:getPositionY()-someroMenuItem:getContentSize().height*0.5*g_fScaleX-5*g_fScaleX
	-- shen_goldIcon:setPosition(posX, posY)
	-- goldFont:setPosition(shen_goldIcon:getPositionX()+shen_goldIcon:getContentSize().width*g_fScaleX+2*g_fScaleX, posY-4*g_fScaleX)
	-- shenIcon:setPosition(goldFont:getPositionX()+goldFont:getContentSize().width*g_fScaleX+1*g_fScaleX, posY+5*g_fScaleX)
	-- -- 拥有神龙令的个数
	-- _shenlonglingNum = ItemUtil.getCacheItemNumBy(_needItemId)
	-- shenNumFont = CCRenderLabel:create(GetLocalizeStringBy("key_2032") .. _shenlonglingNum ..")", g_sFontName, 21, 1, ccc3(0, 0, 0), type_stroke)
	-- shenNumFont:setColor(ccc3(0xff, 0xe4, 0x00))
	-- shenNumFont:setAnchorPoint(ccp(0.5, 1))
	-- local posY = shen_goldIcon:getPositionY()-shen_goldIcon:getContentSize().height*g_fScaleX-5*g_fScaleX
	-- shenNumFont:setPosition(someroMenuItem:getPositionX(),posY)
	-- _bgLayer:addChild(shenNumFont)
	-- shenNumFont:setScale(g_fScaleX)
	...
	lie_goldIcon:setAnchorPoint(ccp(0, 1))
	...
	lie_goldFont:setAnchorPoint(ccp(0, 1))
    ...
    -- 猎十次
    local huntTenMenuItem = CCMenuItemImage:create("images/hunt/huntTen_n.png","images/hunt/huntTen_h.png")
    ...
    huntTenMenuItem:registerScriptTapHandler(huntTenMenuAction)
    ...
    if(isOpne)then
    	...
    	local goldFont = CCRenderLabel:create(GetLocalizeStringBy("key_3149") .. needLeve ..GetLocalizeStringBy("key_2469") , g_sFontName, 21, 1, ccc3(0, 0, 0), type_stroke)
		goldFont:setColor(ccc3(0xff, 0xe4, 0x00))
		goldFont:setAnchorPoint(ccp(0, 1))
		_bgLayer:addChild(goldFont)
		goldFont:setScale(g_fScaleX)
		...
		elseif (fn == "cellAtIndex") then
			...
			for i=1,5 do
				if(_fsoulData[a1*5+i] ~= nil)then
					...
					local iconSprite = ItemSprite.getItemSpriteById( tonumber(_fsoulData[a1*5+i].item_template_id), tonumber(_fsoulData[a1*5+i].item_id),nil,-131)
...
function tenSomeroCallFun( ... )
	...
	local function createNextFun( tItems, tExtra )
		...
		ShowTenGetDialog.showTip(tItems, tExtra)
...
function huntMenuAction( ... )
	...
	local function createNextFun( tItems )
		-- 添加新物品
		addNewFSData( tItems )
		-- 刷新背包
		local itemId = nil
...
function huntTenMenuAction( ... )
	...
	-- 银币不足
	local maxCost = HuntSoulData.getMaxCost()
...
function shenRefreshUI( ... )
	...
	-- 拥有神龙令的个数
	-- if(shenNumFont)then
	-- 	shenNumFont:removeFromParentAndCleanup(true)
	-- 	shenNumFont = nil
	-- end
	-- shenNumFont = CCRenderLabel:create(GetLocalizeStringBy("key_2032") .. _shenlonglingNum ..")", g_sFontName, 21, 1, ccc3(0, 0, 0), type_stroke)
	-- shenNumFont:setColor(ccc3(0xff, 0xe4, 0x00))
	...
	if(_liziName)then
		...
		local function fnliziAnimSprite1End( ... )
			if(liziAnimSprite1)then
				...
				-- 神龙特效屏蔽层去除
				-- if(shenLongMaskLayer)then
				-- 	shenLongMaskLayer:removeFromParentAndCleanup(true)
				-- 	shenLongMaskLayer = nil
				-- end
			end
		end
...
function createShenLong( ... )
	...
	-- end
	-- local runningScene = CCDirector:sharedDirector():getRunningScene()
	-- shenLongMaskLayer = BaseUI.createMaskLayer(-5000,nil,nil,0)
	-- runningScene:addChild(shenLongMaskLayer, 10000)
