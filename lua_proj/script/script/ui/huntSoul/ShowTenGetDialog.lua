...
function createCell( p_data, p_index )
	...
	if(table.isEmpty(p_data))then
		-- 第1行 额外获得中秋道具
		local str = string.format(GetLocalizeStringBy("lic_1222"), index)
		...
		if(not table.isEmpty(_extraData))then
			...
			for k,v in pairs(_extraData) do
				...
				font2:setPosition(ccp(10,font1:getPositionY()-font1:getContentSize().height-10*i+font2:getContentSize().height*(i-1)))
				cell:addChild(font2)
			end
		else
			local str = string.format(GetLocalizeStringBy("lic_1223"), index)
	...
	-- 战魂，
	-- local zhanStr = GetLocalizeStringBy("lic_1155")
	...
	-- -- 战魂，
	-- local zhanFont = CCLabelTTF:create(zhanStr,g_sFontName,23)
	-- zhanFont:setColor(ccc3(0xff,0xff,0xff))
	-- zhanFont:setAnchorPoint(ccp(0,1))
	-- zhanFont:setPosition(ccp(itemName:getPositionX()+itemName:getContentSize().width,font2:getPositionY()))
	-- cell:addChild(zhanFont)
	...
	if(p_data[3])then
		...
		-- zhanFont:setColor(ccc3(0xff,0xff,0xff))
		-- zhanFont:setAnchorPoint(ccp(0,1))
		-- zhanFont:setPosition(ccp(itemName:getPositionX()+itemName:getContentSize().width,font3:getPositionY()))
		-- cell:addChild(zhanFont)
	end
...
--[[
	...
	@return :
--]]
function showTip( p_items, p_tExtra )
	-- 初始化
	init()
