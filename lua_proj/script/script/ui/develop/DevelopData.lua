...
function getHeroInfo( p_hid )
	...
	-- local fightForceData = HeroModel.getHeroAffix(p_hid)
	-- --统帅
...
function doOpenDevelopByHtid( p_htid )
	...
	local temp = DB_Heroes.getDataById(p_htid)
	print("doOpenDevelopByHtid", p_htid, openOrangeHeroIds, configInfo, temp)
...
--[[
	@des 	:在拥有的武将中根据p_htid筛选不小于等级p_level的英雄
			 1. 不能包含被进化的英雄
			 2. 英雄模版为材料模版
			 3. 英雄等级不小于要求等级
			 4. 英雄进阶等级不能大于0
...
function getHeroesByHtid( p_hid, p_htid, p_level )
	...
	for k,v in pairs(allHeroes) do
		if tonumber(v.hid) ~= p_hid and tonumber(v.htid) == p_htid and
		   tonumber(v.level) <= p_level and tonumber(v.evolve_level) == 0 and
		   not HeroPublicLua.isBusyWithHid(v.hid) and not LittleFriendData.isInLittleFriend(v.hid) then
