...
function createIcon( p_index )
	...
	if data.type == DevelopData.kItemTag then
		icon = ItemSprite.getItemSpriteById(data.id,nil,nil,nil,kMenuPriority+1)
	elseif data.type == DevelopData.kHeroTag then
		--icon = HeroUtil.getHeroIconByHTID(data.id)
		icon = ItemSprite.getHeroIconItemByhtid(data.id, kMenuPriority+1)
	else
...
function createUI( ... )
	...
	for i = 1,2 do
		...
		_cardTable[i]:setAnchorPoint(ccp(0.5,0))
		_cardTable[i]:setPosition(cardData[i][3], kAdaptiveSize.height-345)
		...
		_attrTable[i]:setPosition(cardData[i][3], 260)
	...
	local btnData = {
		...
		[1] = {CreateUI.createScale9MenuItem("images/common/btn/btn_blue_n.png","images/common/btn/btn_blue_h.png", nil,
			   CCSizeMake(168,65), GetLocalizeStringBy("zz_84"),28), tapInfoBtnCb, ccp(501,kAdaptiveSize.height-405)},
...
function tapDevelopBtnCb( p_tag, p_item )
	...
	AlertTip.showAlert(GetLocalizeStringBy("zz_102"), tapComfirmBtnCb,true)
end
