...
-------------- 赠送礼物 ----------
	...
	progressSp:setContentSize(CCSizeMake(520 * levelExp/needExp, 23))
...
function createStarSprite()
	-- 名将全身像
	_curStarBodyImage = StarSprite.createStarSprite( _allStarInfoArr[_curStarIndex].star_tid )
	_curStarBodyImage:setAnchorPoint(ccp(0.5, 0))
	_curStarBodyImage:setPosition(ccp( _bgLayer:getContentSize().width * 0.5, _bgLayer:getContentSize().height * _starScaleY))
...
local function createStarAttr()
	...
	_starAttrSprite:addChild(menuBar, 10)
    ...
    menuBar:addChild(hiddenMenuItem, 2, 81002)
...
function createUI()
	...
	exchangeBtn:setPosition(MainScene.getMenuPositionInTruePoint(_bgLayer:getContentSize().width*190/640, _bgLayer:getContentSize().height*870/960))
...
local function switchNextStar( xOffset )
	...
	if( nextStarIndex >= 1 and  nextStarIndex <= #_allStarInfoArr) then
		local nextStarSprite = StarSprite.createStarSprite( _allStarInfoArr[nextStarIndex].star_tid )
