...
function createStarExchangeNode( p_curDisStarId )
	...
	-- 头像icon
	local iconSprite1 = HeroUtil.getHeroIconByHTID(_curSrcStarData.star_tid)
	...
	if( p_curDisStarId )then
		...
		-- 头像icon
		local heroIconSprite = HeroUtil.getHeroIconByHTID(disStarData.star_tid)
		...
		elseif fn == "cellAtIndex" then
			...
			for i=1,5 do
				if (start + i <= #_curStarList) then
					...
					local iconSprite = StarSprite.createIconButton( _curStarList[start + i].star_tid, _curStarList[start + i].level, _curStarList[start + i].star_id, isNeedLine, clickBtnAction )
