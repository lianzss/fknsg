...
function createMemberCell(memberInfo, isChecked, delegateFunc)
	...
	levelSprite:setPosition(ccp(158, name_level_bg:getContentSize().height*0.5))
	name_level_bg:addChild(levelSprite)

	-- 等级
	local levelLabel = CCLabelTTF:create(memberInfo.level, g_sFontName, 21)
	levelLabel:setPosition(193, name_level_bg:getContentSize().height*0.5)
	...
	if(isChecked == true)then
		...
		-- 
		name_level_bg:setPosition(ccp(30, 175))
		-- 战斗力图标
		local fightSprite = CCSprite:create("images/common/fight_value02.png")
		fightSprite:setAnchorPoint(ccp(0, 0.5))
		fightSprite:setPosition(ccp(285, 170))
		cellBg:addChild(fightSprite)
		-- 战斗力
	    ...
	    fightLabel:setColor(ccc3(0xff, 0xf6, 0x00))
	   	fightLabel:setAnchorPoint(ccp(0,0.5))
	    fightLabel:setPosition(ccp(365, 165))
	    cellBg:addChild(fightLabel)
	    ...
	    ontimeLabel:setPosition(ccp(600, 170))
	    ...
	    for k,t_text_label in pairs(t_label_arr) do
	    	t_text_label:setPosition(530-x_offset*0.5 + t_offset, 30)
	    ...
	    leftFireTimesTitleLabel:setPosition(ccp(580, 140))
	    ...
	    if(tonumber(memberInfo.member_type) == 0)then
		    ...
		    posLabel:setColor(ccc3(0xff, 0xff, 0xff))
		   	posLabel:setAnchorPoint(ccp(0.5,0.5))
		    posLabel:setPosition(ccp(75, 45))
			...
			posSprite:setPosition(ccp(75, 40))
		...
		jinjiSpriteBg:setPosition(ccp(260, 105))
		cellBg:addChild(jinjiSpriteBg)

	    local jinjiTitleLabel = CCRenderLabel:create(GetLocalizeStringBy("key_1670"), g_sFontPangWa, 23, 1, ccc3( 0x00, 0x00, 0x00), type_stroke)
	    jinjiTitleLabel:setColor(ccc3(0xff, 0xe4, 0x00))
	   	jinjiTitleLabel:setAnchorPoint(ccp(0.5,0.5))
	    jinjiTitleLabel:setPosition(ccp(205, 107))
	    cellBg:addChild(jinjiTitleLabel)
	    -- 排名
	    local rankText = nil
	    if(memberInfo.position) then
	    ...
	    donateTotalTitleLabel:setPosition(ccp(255, 67))
		...
		donateTotalBg:setPosition(ccp(260, 65))
	    ...
	    if(tonumber(memberInfo.uid) == UserModel.getUserUid())then
	    	-- 竞技排名
	    	jinjiTitleLabel:setPosition(ccp(205, 118))
	    	jinjiSpriteBg:setPosition(ccp(260, 116))
	    	-- 总贡献
	    	donateTotalTitleLabel:setPosition(ccp(255, 80))
	    	donateTotalBg:setPosition(ccp(260, 78))
		    ...
		    donateTitleLabel:setPosition(ccp(205, 42))
			...
			donateBg:setPosition(ccp(260, 40))
		    ...
		    donateLabel:setPosition(ccp(265, 42))
	    ...
	    if(tonumber(memberInfo.member_type) == 1 and tonumber(memberInfo.uid) == UserModel.getUserUid())then
			...
			managerBtn:setPosition(ccp(530, 90))
