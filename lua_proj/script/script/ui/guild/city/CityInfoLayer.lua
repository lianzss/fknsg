...
function initCityInfoLayer( ... )
	...
	_backGround:addChild(menuBar,3)
 	local lookButton = LuaCC.create9ScaleMenuItem("images/common/btn/btn_purple2_n.png","images/common/btn/btn_purple2_h.png",CCSizeMake(200, 73),GetLocalizeStringBy("key_1240"),ccc3(0xfe, 0xdb, 0x1c),35,g_sFontPangWa,1, ccc3(0x00, 0x00, 0x00))
	...
	if( guildTaskType == 2)then
		...
		-- 消耗体力5
		local tipLable = CCRenderLabel:create(GetLocalizeStringBy("lic_1073"), g_sFontName,18,1, ccc3(0x00, 0x00, 0x00), type_stroke)
		tipLable:setColor(ccc3(0x00,0xff,0x18))
		tipLable:setAnchorPoint(ccp(0.5,1))
		tipLable:setPosition(ccp(breakButton:getContentSize().width*0.5,2))
		breakButton:addChild(tipLable)
		-- 破坏城防cd时间
		local cdLable = CCLabelTTF:create("00:00:00", g_sFontName,24)
		cdLable:setColor(ccc3(0x0e,0x79,0x00))
		cdLable:setAnchorPoint(ccp(0.5,1))
		cdLable:setPosition(ccp(breakButton:getContentSize().width*0.5,-20))
		breakButton:addChild(cdLable)
		cdLable:setVisible(false)
			...
			if( TimeUtil.getSvrTimeByOffset() < disTime )then
				...
				local function showTimeDown( ... )
					if(disTime - TimeUtil.getSvrTimeByOffset() < 0)then
						breakButton:stopAllActions()
						cdLable:setVisible(false)
						return
					end
					cdLable:setString(TimeUtil.getTimeString(disTime - TimeUtil.getSvrTimeByOffset()))
					local actionArray = CCArray:create()
					actionArray:addObject(CCDelayTime:create(1))
					actionArray:addObject(CCCallFunc:create(showTimeDown))
					breakButton:runAction(CCSequence:create(actionArray))
				end
				cdLable:setVisible(true)
				showTimeDown()
			end
		end
		-- 按钮坐标
	...
	elseif( guildTaskType == 1)then
		...
		tipLable:setColor(ccc3(0x00,0xff,0x18))
		tipLable:setAnchorPoint(ccp(0.5,1))
		tipLable:setPosition(ccp(repairButton:getContentSize().width*0.5,2))
		repairButton:addChild(tipLable)
		-- 修复城防cd时间
		local cdLable = CCLabelTTF:create("00:00:00", g_sFontName,24)
		cdLable:setColor(ccc3(0x0e,0x79,0x00))
		cdLable:setAnchorPoint(ccp(0.5,1))
		cdLable:setPosition(ccp(repairButton:getContentSize().width*0.5,-20))
		repairButton:addChild(cdLable)
		cdLable:setVisible(false)
		...
		if(lastTime ~= 0)then
			...
			if( TimeUtil.getSvrTimeByOffset() < disTime )then
				...
				local function showTimeDown( ... )
					...
					actionArray:addObject(CCDelayTime:create(1))
					actionArray:addObject(CCCallFunc:create(showTimeDown))
					repairButton:runAction(CCSequence:create(actionArray))
				end
				cdLable:setVisible(true)
				showTimeDown()
			end
...
function showTimeStepTip()
	...
	if(TimeUtil.getSvrTimeByOffset() >= tonumber(timesInfo.arrAttack[1][1]) - timesInfo.prepare - 600 and TimeUtil.getSvrTimeByOffset() < tonumber(timesInfo.arrAttack[1][1]) )then
		isShow = false
	...
	if(TimeUtil.getSvrTimeByOffset() >= tonumber(timesInfo.arrAttack[2][2]+600) and TimeUtil.getSvrTimeByOffset() < tonumber(timesInfo.rewardStart))then
		isShow = true
		print("dade")
		if(breakButton~=nil)then
			breakButton:setVisible(true)
	...
	elseif(TimeUtil.getSvrTimeByOffset()>=tonumber(timesInfo.rewardStart))then
		isShow = true
		if(breakButton~=nil)then
			breakButton:setVisible(true)
		end
...
function breakCallBack( cbFlag, dictData, bRet )
	...
	BattleLayer.showBattleWithString(dictData.ret.atk.fightRet, doMatchBattleOverDelegate, MissionAfterBattle.createAfterBattleLayer(dictData.ret.atk,true,dobattleCallback,true,cityName), "ducheng.jpg")
end
...
function repairCallBack( cbFlag, dictData, bRet )
	...
	require "script/battle/BattleLayer"
	BattleLayer.showBattleWithString(dictData.ret.atk.fightRet, doMatchBattleOverDelegate, MissionAfterBattle.createAfterBattleLayer(dictData.ret.atk,true,dobattleCallback,false,cityName), "ducheng.jpg")
end
...
-- 保存本次破坏城防时间
function saveBreakCDTime( p_time )
	CCUserDefault:sharedUserDefault():setIntegerForKey("break_" .. UserModel.getUserUid(), tonumber(p_time))
	CCUserDefault:sharedUserDefault():flush()
end

-- 得到上次破坏城防时间
function getBreakCDTime( )
	return CCUserDefault:sharedUserDefault():getIntegerForKey("break_" .. UserModel.getUserUid())
end

-- 保存本次修复城防时间
function saveRepairCDTime( p_time )
	CCUserDefault:sharedUserDefault():setIntegerForKey("repair_" .. UserModel.getUserUid(), tonumber(p_time))
	CCUserDefault:sharedUserDefault():flush()
end

-- 得到上次修复城防时间
function getRepairCDTime( )
	return CCUserDefault:sharedUserDefault():getIntegerForKey("repair_" .. UserModel.getUserUid())
end
