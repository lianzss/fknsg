...
local Tag_High_Build 	= 10003
local Tag_Super_Build	= 10004
...
function createBuildSrcowView( )
	...
	_rewardTagTable = {Tag_Normal_Build , Tag_Middle_Build , Tag_High_Build,Tag_Super_Build}
	_buildSpriteTab= {}
	-- scrollLayer:setContentSize(CCSizeMake(_bgSprite:getContentSize().width,335))
	scrollLayer:setPosition(ccp(0,0))

	local scrWidth=_bgSprite:getContentSize().width*0.322*4/MainScene.elementScale
...
function createContributeTableView( ... )
	...
	local handler = LuaEventHandler:create(function(fn, table, a1, a2)
		...
		if (fn == "cellSize") then
			-- 显示单元格的间距
			local interval = 5
			r = CCSizeMake(cell_bg_size.width, (cell_bg_size.height + interval))
...
function buildCallback(  cbFlag, dictData, bRet  )
	if( dictData.ret == "ok")then
		...
		tolua.cast(_buildSpriteTab[3]:getChildByTag(100):getChildByTag( Tag_High_Build ),"CCMenuItemSprite"):setEnabled(false)
		tolua.cast(_buildSpriteTab[3]:getChildByTag(100):getChildByTag( Tag_High_Build ):getChildByTag(10),"CCLabelTTF"):setColor(ccc3(0xf1,0xf1,0xf1))
		tolua.cast(_buildSpriteTab[4]:getChildByTag(100):getChildByTag( Tag_Super_Build ),"CCMenuItemSprite"):setEnabled(false)
		tolua.cast(_buildSpriteTab[4]:getChildByTag(100):getChildByTag( Tag_Super_Build ):getChildByTag(10),"CCLabelTTF"):setColor(ccc3(0xf1,0xf1,0xf1))
...
function refreshAllLable( ... )
	-- 刷新银币
	if(silverLabel)then
		silverLabel:setString( UserModel.getSilverNumber() )
	end
	-- 刷新金币
	if(goldLabel)then
		goldLabel:setString( UserModel.getGoldNumber() )
