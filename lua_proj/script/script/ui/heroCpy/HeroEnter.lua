...
function showLayer(htidCpy)
 ...
 --    bgSprite:addChild(lieLabel)
	...
	--第七天赋label
	sevenBilyLabel = CCRenderLabel:create( "通关" .. GetLocalizeStringBy("lic_" .. (1035 + copyLevel)) .. "难度武将列传可领悟" , g_sFontPangWa, 23, 1.5, ccc3( 0x00, 0x00, 0x00), type_stroke)
...
function updateLabel( ... )
	sevenBilyLabel:setString("通关" .. GetLocalizeStringBy("lic_" .. (1035 + copyLevel)).. "难度武将列传可领悟")
...
function startStory( ... )
	...
	-- 	AnimationTip.showTip("已通关该难度武将列传副本，武将进阶+".. evolveLevel .. "后可领悟" ..GetLocalizeStringBy("lcy_" .. (1001 + copyLevel)))
	-- 	return
	-- end
