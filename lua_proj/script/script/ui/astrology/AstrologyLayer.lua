...
function refreshClick(tag,menuItem,isForceRefresh)
    ...
    if(UserModel.getGoldNumber()<10 and tonumber(m_astrologyInfo.free_refresh_num)<=0 and tonumber(havaCostNum) - tonumber(m_cost_num) <0)then
        ...
        local function refreshGold()
            ...
            m_silverLabel:setString(tostring(userInfo.silver_num))
...
function astroClick(tag, item_obj)
    ...
    effectActionArray:addObject(CCMoveTo:create(1,ccp(currentX,layerSize.height*0.54)))
        ...
        if(m_astrologyInfo.va_divine.lighted[i]==nil or tonumber(m_astrologyInfo.va_divine.lighted[i])==0)then
            for j=1,5 do
                ...
                if(starId~=nil and starId == m_astrologyInfo.va_divine.target[i] and m_astrologyMenu:getChildByTag(1000+j)~=nil)then
                    ...
                    m_astrologyMenu:getChildByTag(1000+j):addChild(m_barEffect,1);
                end
            end
        end
    end
...
function initAstrologyLayer()
    ...
    local descButton = LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png",CCSizeMake(210,73),GetLocalizeStringBy("key_3022"),ccc3(255,222,0))
    descButton:setAnchorPoint(ccp(0.5,0.5))
    descButton:setPosition(MainScene.getMenuPositionInTruePoint(layerSize.width*0.2,layerSize.height*0.39))
    m_astrologyMenu:addChild(descButton)
    descButton:registerScriptTapHandler(showDesc)
    ...
    rewardButton:setPosition(MainScene.getMenuPositionInTruePoint(layerSize.width*0.8,layerSize.height*0.39)) 
