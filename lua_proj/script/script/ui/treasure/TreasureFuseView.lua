...
function create( treasure_type, p_layerSize )
	...
	-- setAdaptNode(fuseButton)

	--碎片数量
	--[[
	local fragmentWordSprite = CCSprite:create(ImagePath .. "fragment.png")
	fragmentWordSprite:setAnchorPoint(ccp(1, 1))
	fragmentWordSprite:setPosition(ccp(0.85 * layerSize.width/MainScene.elementScale, 0.9 *layerSize.height/MainScene.elementScale))
	mainLayer:addChild(fragmentWordSprite)

	fragmentCountLabel = CCRenderLabel:create(tostring(TreasureData.getFragmentCount(treasureType)), g_sFontPangWa, 28, 0, ccc3( 0x00, 0x00, 0x00), type_stroke)
	fragmentCountLabel:setColor(ccc3(0x00, 0xe4, 0xff))
	fragmentCountLabel:setPosition(fragmentWordSprite:getPositionX() + 5/MainScene.elementScale, fragmentWordSprite:getPositionY())
	mainLayer:addChild(fragmentCountLabel)
	]]
...
function createTreasureFragmentSet( treasure_id )
	...
	iconItemSprite:registerScriptTapHandler(function ( ... )
		...
		if ItemUtil.isExpTreasById(treasure_id,nil) then
			treasInfoLayer = TreasInfoLayer.createLayer( treasure_id,  nil, false, false, false, function ( ... )
				iconItemSprite:setEnabled(true)
			end, nil, nil, -621, 99)
		else
			treasInfoLayer = NewTreasInfoLayer.createLayer( treasure_id,  nil, false, false, false, function ( ... )
				iconItemSprite:setEnabled(true)
			end, nil, nil, -621, 99)
		end
		local runningScene = CCDirector:sharedDirector():getRunningScene()
		runningScene:addChild(treasInfoLayer, 2000)
...
function fuseButtonCallback( tag,sender )
	...
	local callbackFunc = function ( isSuccess )
		...
		function lightningEndedCallFunc(  )
    		...
    		require "script/ui/item/ItemUtil"
    		local treasInfoLayer
    		if ItemUtil.isExpTreasById(selectTreasureId,nil) then
    			treasInfoLayer = TreasInfoLayer.createLayer( selectTreasureId,  nil, false, false, false, closeCallFunc, nil, nil, -621, 99)
    		else
				treasInfoLayer = NewTreasInfoLayer.createLayer( selectTreasureId,  nil, false, false, false, closeCallFunc, nil, nil, -621, 99)
			end
			local runningScene = CCDirector:sharedDirector():getRunningScene()
			runningScene:addChild(treasInfoLayer, 2000)
	...
	local goOkFunc = function ( ... )
		if(isExcuteService == false) then
			TreasureService.fuse(selectTreasureId, callbackFunc)
...
function addNewGuideStepTreasureInfo( ... )
	local guideFunc = function ( ... )
        ...
        if(NewGuide.guideClass ==  ksGuideRobTreasure and RobTreasureGuide.stepNum == 7) then
            -- RobTreasureGuide.changLayer(0)
            require "script/ui/item/NewTreasInfoLayer"
            local robTreasure =  NewTreasInfoLayer.getGuideObject_2()
            local touchRect   = getSpriteScreenRect(robTreasure)
