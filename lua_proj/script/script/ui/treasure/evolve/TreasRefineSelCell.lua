...
function createCell(treasData ,callbackfn)
	...
	local  function showTreasureInfoLayer( tag,sender )
		...
		if ItemUtil.isExpTreasById(tonumber(treasData.item_template_id),tonumber(treasData.item_id)) then
			treasInfoLayer = TreasInfoLayer.createLayer(tonumber(treasData.item_template_id), tonumber(treasData.item_id) , nil, nil, nil, nil, nil, nil, -512)
		else
			treasInfoLayer = NewTreasInfoLayer.createLayer(tonumber(treasData.item_template_id), tonumber(treasData.item_id) , nil, nil, nil, nil, nil, nil, -512)
		end
		local runningScene = CCDirector:sharedDirector():getRunningScene()
		runningScene:addChild(treasInfoLayer, 3000)
	end

	local norSprite  =  ItemSprite.getItemSpriteByItemId(tonumber(treasData.item_template_id)) 
	local selSprite  =  ItemSprite.getItemSpriteByItemId(tonumber(treasData.item_template_id)) 
