...
function createContent( ... )
	...
	local leftCardSpriteA = TreasCardSprite.createSprite(treasureInfo.item_template_id)
	local leftCardSpriteB = TreasCardSprite.createSprite(treasureInfo.item_template_id)
	...
	local rightCard = TreasCardSprite.createSprite(treasureInfo.item_template_id)
...
function createMenu( ... )
	...
	local closeButton = LuaCC.create9ScaleMenuItem("images/common/btn/btn1_d.png","images/common/btn/btn1_n.png", CCSizeMake(210,73), GetLocalizeStringBy("key_1951"), ccc3(255,222,0))
...
function createOldInfo( ... )
	...
	nameLabel:setColor(HeroPublicLua.getCCColorByStarLevel(tonumber(treasureInfo.itemDesc.quality)))
...
function closeButtonCallback( ... )
	...
	elseif(parentLayerTag == kTreasureListTag) then
		...
		local bagLayer = BagLayer.createLayer(BagLayer.Tag_Init_Treas, BagLayer.Type_Bag_Prop_Treas)
...
function createEvolveGemPanel( level,maxLevel )
	local row 		= math.floor(maxLevel/5)
	...
	for i=0, maxLevel-1 do
		...
		if(i <= tonumber(level-1)) then
			sprite 	= CCSprite:create("images/common/big_gem.png")
		else
			sprite 	= CCSprite:create("images/common/big_gray_gem.png")
		end
		sprite:setAnchorPoint(ccp(0.5, 0))
		local dis  	= gemPanle:getContentSize().width/5
		local x    	= dis/2 + dis * (i%5)
		local y 	= gemPanle:getContentSize().height - (math.floor(i/5) + 1)*35
