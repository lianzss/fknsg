...
function fnCreateTransferEffect( tparam)
	...
	local csCardShow = TreasCardSprite.createSprite(tonumber(tparam.item_temple_id), 0) --HeroPublicCC.createSpriteCardShow( DestinyData.changeHeroHtid() )
...
function getDiamondSprite(limitLv, curWasterLv )
	...
	for i=1, limitLv do
		...
		if(i<= tonumber(curWasterLv)) then
			local gemSprite= CCSprite:create("images/common/big_gem.png")
