	...
	if(BTUtil:getPlatform() == kBT_PLATFORM_ANDROID) then
		...
		if(WXAppInstalled) then
			...
			if(WXAppSupportAPI) then
				...
				AnimationTip.showTip("当前微信版本不支持朋友圈分享")
			end
		else
			AnimationTip.showTip("您还没有安装微信,请安装后再分享")
		end
	else
...
function show(p_weiboText, p_shareImage,p_zIndex, p_touchPriority, callback)
	...
	local aleteNode = BaseUI.createHorizontalNode({explainLabel21, silverSprite22, silverNum23})
...
function requestCallbackFunc( cbFlag, dictData, bRet )
	if(bRet == true) then
		...
		if(dictData.ret.gold ~= nil) then
			str =str .. GetLocalizeStringBy("key_1984") .. dictData.ret.gold .. GetLocalizeStringBy("key_1062")
			UserModel.addGoldNumber(tonumber(dictData.ret.gold))
		end
		if(dictData.ret.silver ~= nil) then
			str =str .. GetLocalizeStringBy("key_1984") .. dictData.ret.silver .. GetLocalizeStringBy("key_1308")
...
function shareWeixinAtMenu( ... )
	...
	if(BTUtil:getPlatform() == kBT_PLATFORM_ANDROID) then
		...
		elseif(shareData.wenxinImage ~= nil) then
