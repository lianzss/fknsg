...
function fnHandlerOfReturn(tag, item_obj)
	...
	printTable("tArgs",tArgs)
	printTable("allGoodTable",allGoodTable)
	printTable("_tParentParam",_tParentParam)
	printTable("_arrSelectedGoods",_arrSelectedGoods)
	...
	if(table.count(tArgs.selectedHeroes) < _selectedMaxCount) then
		AnimationTip.showTip("提交的物品数量不足")
		return
	end
	fnHandlerOfClose()
...
function getFiltersForItem()
	...
	for k,v in pairs(bagInfo.arm) do
		print("v.itemDesc.item_type===="..v.itemDesc.type)
		print("typeCpy===="..typeCpy)
		print("v.itemDesc.quality======"..v.itemDesc.quality)
		print("qualityCpy======"..qualityCpy)
