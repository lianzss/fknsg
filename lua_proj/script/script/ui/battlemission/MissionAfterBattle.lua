...
local function userFormationItemFun( tag, item_obj )
    ...
    -- else
        ...
        local fightData = {}
        fightData.server = _allData.fightRet
...
function createAfterBattleLayer( tAllData, isReplay, CallFun,isDestory,cityName)
    ...
    _isReplay = isReplay
    print("wawawwwa",cityName)
    local cityNameCpy = cityName

    local amf3_obj = Base64.decodeWithZip(tAllData.fightRet)
    ...
    if(tonumber(tAllData.uid)~=0)then
