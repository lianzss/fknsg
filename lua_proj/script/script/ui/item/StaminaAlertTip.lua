...
local curHasLabel = nil
local curNumber = 0
local alertBg
local _curPrice = 0
local curPriceLabel = nil
local fnStaminaNumberAdd = nil     -- 耐力增加后回调函数
local fuBuyStaminaCallFun = nil    -- 购买耐力后回调
...
local function init()
	...
	_curPrice = 0
	curPriceLabel = nil
	fnStaminaNumberAdd = nil
	fuBuyStaminaCallFun = nil
end
...
function buyCallback( cbFlag, dictData, bRet )
	if(dictData.err == "ok") then
		...
		curHasLabel:removeFromParentAndCleanup(true)
		curHasLabel = nil
		curNumber = curNumber + 1
		curHasLabel = CCRenderLabel:create(GetLocalizeStringBy("key_3413") .. curNumber, g_sFontName, 20, 1, ccc3(0x00, 0x00, 0x00), type_stroke)
	    curHasLabel:setColor(ccc3(0xfe, 0xdb, 0x1c))
	    curHasLabel:setAnchorPoint(ccp(0,0))
	    curHasLabel:setPosition(ccp(alertBg:getContentSize().width*0.59, alertBg:getContentSize().height*0.07) )

	    -- 修改当前价格
	    curPriceLabel:removeFromParentAndCleanup(true)
	    -- 现价
		_curPrice = ShopUtil.getNeedGoldByGoodsAndTimes( goodsData.id, ShopUtil.getBuyNumBy(goodsData.id)+1)
		curPriceLabel = CCRenderLabel:create(_curPrice, g_sFontName, 20, 1, ccc3(0x00, 0x00, 0x00), type_stroke)
	    curPriceLabel:setColor(ccc3(0xfe, 0xdb, 0x1c))
	    curPriceLabel:setAnchorPoint(ccp(0,0))
	    curPriceLabel:setPosition(ccp(alertBg:getContentSize().width*0.3, alertBg:getContentSize().height*0.07) )
	    alertBg:addChild(curPriceLabel)

	    alertBg:addChild(curHasLabel)
	    -- 购买后回调
	    if(fuBuyStaminaCallFun ~= nil)then
	    	fuBuyStaminaCallFun()
	   	end
	end
end
...
function useItemCallback( cbFlag, dictData, bRet )
	if (dictData.err == "ok") then
		...
		-- 注册方法不为空则调用
		print("fnStaminaNumberAdd--",fnStaminaNumberAdd)
		if( fnStaminaNumberAdd ~= nil )then
			fnStaminaNumberAdd()
		end
	end
	if(alertLayer) then
...
--[[
	...
	@return void
--]]
function showTip(func1,func2)
	init()
	-- 耐力增加后回调
	print("func",func)
	fnStaminaNumberAdd = func1
	fuBuyStaminaCallFun = func2
