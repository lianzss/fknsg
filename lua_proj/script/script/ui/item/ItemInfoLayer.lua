...
function createItemInfoLayer(item_id, item_template_id,menu_property)
	...
	-- 显示名称
	local nameLabel = CCRenderLabel:create(_itemData.name,  g_sFontName , 28, 2, ccc3( 0x00, 0x00, 0x00), type_stroke)
	...
	if(_itemData.desc ~= nil ) then 
		local descLabel = CCLabelTTF:create(_itemData.desc, g_sFontName, 24,CCSizeMake(345,90),kCCTextAlignmentLeft)
	...
	-- 数量
	if(_itemNum ~= 0) then
		-- numberLabel = CCLabelTTF:create(GetLocalizeStringBy("key_1486") .. _itemNum ,g_sFontName,24)
		numberLabel= CCRenderLabel:createWithAlign(GetLocalizeStringBy("key_1486") .. _itemNum ,g_sFontName,24,1,ccc3(0,0,0), type_stroke,CCSizeMake(330,30),kCCTextAlignmentLeft,kCCVerticalTextAlignmentTop)
