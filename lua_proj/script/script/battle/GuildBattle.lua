    ...
    if(team1Info.members~=nil)then
        for key, value in pairs(team1Info.members) do
            if(teamId==key)then
                ...
                local resultCard         = BattleCardUtil.getBattlePlayerCard(0,false,htid,false,replaceImage)
...
local function showReport()
    if(m_battleType == BattleForCity) then
        ...
        GuildBattleReportLayer.showLayer(m_battleData,true,-490,11111,true)
    else
        require "script/ui/guild/copy/GuildBattleReportLayer"
        GuildBattleReportLayer.showLayer(m_battleData,true,-490,11111)
    end
...
function createLayer(battleData, battleType, afterBattleLayer, isReport)
    ...
    battleBaseLayer:registerScriptTouchHandler(layerTouch,false,-488,true)
