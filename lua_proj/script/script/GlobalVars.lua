...
g_publish_version = nil

if(NSBundleInfo)then
    g_publish_version = NSBundleInfo:getAppVersion()
else
    g_publish_version=CCUserDefault:sharedUserDefault():getStringForKey("BT_GP_Version")
    g_publish_version=string.gsub(g_publish_version, " ", "")
    if(g_publish_version == nil or "" == g_publish_version)then
        g_publish_version = "1.1.9"
    end
end
...
local function fnSelectFont( ... )
    if g_system_type == kBT_PLATFORM_ANDROID then
        ...
        --如果是系统是SCH-i909, 或者系统版本是2.3以下不支持自定义字体
        if(NSBundleInfo)then
