...
function getHashUrl( )
	local hashUrl = "http://mapifknsg.zuiyouxi.com/phone/getHash/"
	local uuid = Platform.getSdk():callStringFuncWithParam("getUuid",nil)
...
function getLoginUrl( username,password )
	...
	local loginUrl = "http://mapifknsg.zuiyouxi.com/phone/login/?".. Platform.getUrlParam().."&action=login&username=" .. username .. "&password=" .. password .. "&ext=" .. "&bind=" .. g_dev_udid .."&other_pl=" .. getOther_pl()
	local uuid = Platform.getSdk():callStringFuncWithParam("getUuid",nil)
	if(uuid ~= nil)then
